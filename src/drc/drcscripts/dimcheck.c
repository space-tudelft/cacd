...
char *argv0 = "dimcheck2";
char *usage = "\nUsage: %s [-c|-n] [-f] [-g] [-t] [-v] [cell_name]\n\n";
...
int main (int argc, char *argv[])
{
	...
	for (i = 1; i < argc && argv[i][0] == '-'; ++i) {
		...
		case 'c':
		case 'n':
			if ((*p == 'c' && nflag) || (*p == 'n' && cflag)) {
			    fprintf(stderr, "\noptions -c and -n are mutually exclusive");
			    fprintf(stderr, usage, argv0);
			    return (1);
			}
			if (*p == 'c') cflag = 1; else nflag = 1;
			break;
		case 'f':
			fflag = 1;
			break;
