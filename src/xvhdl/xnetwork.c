...
void xnetwork (struct cir *cl)
{
    ...
    struct net_ref *nets;
    char ofname[DM_MAXNAME + 5];

    outPos ();

    sprintf (ofname, "%s.nmp", cl -> name);
    fp_nmp = fopen (ofname, "r");
    if (fp_nmp && test_nmp ()) fatalErr ("illegal format:", ofname);
    if (fp_nmp) fprintf (fp_out, "\n-- using mapfile: %s\n", ofname);

    ntw = newNetw (cl -> name, cl -> proj, cl -> imported, cl -> orig_name, cl -> next != NULL);
    ...
    if (fp_nmp) { fclose (fp_nmp); fp_nmp = NULL; }
