...
void c_label (Coor x, Coor y, int orient)
{
    ...
    len = ask_string ("enter_label: ", buf);
    ...
    if (!*p1) {
	btext ("Enter label as \"name[:class[:mask]]\"");
	goto err;
    }
	...
	if (maskno < 0) {
	    btext ("Illegal mask in \"name:class:mask\"!");
	    goto err;
	}
    }
    ...
    LABELOBJ.y = y;
    add_comment (element);
    if (maskno < NR_lay && !ggTestColor (maskno, "black")) pict_arr[maskno] = DRAW;
    pict_arr[Textnr] = DRAW;
    return;
err:
    sleep (1);
...
static void del_element (Comment *element)
{
	...
	if (maskno >= 0 && maskno < NR_lay && !ggTestColor (maskno, "black")) pict_arr[maskno] = ERAS_DR;
    }
}
...
void draw_all_comments ()
{
    ...
    if (!vis_arr[v_comment] && !vis_arr[v_label]) return; /* not visible */

    for (element = firstElement; element; element = element -> next) {
	switch (element -> type) {
	case GA_LINE:
	    if (!vis_arr[v_comment]) break;
	...
	case GA_TEXT:
	    ...
	    break;
	case GA_LABEL:
	    if (!vis_arr[v_label]) break;
	    maskno = LABELOBJ.maskno;
	    if (maskno >= 0 && maskno < NR_lay) {
		if (ggTestColor (maskno, "black")) {
		    coors[0] = coors[2] = coors[4] = LABELOBJ.x;
		    ++coors[2];
		    coors[1] = coors[3] = coors[5] = LABELOBJ.y;
		    ++coors[5];
		    pict_poly (coors, 3);
		}
		sprintf (buf, "%s:%s", LABELOBJ.name, LABELOBJ.class);
	    }
	    else {
		/* In some special cases, space may generate labels
...
/*
** Draw label layers of the cell (highest level).
*/
void dis_label (int lay, Coor wxl, Coor wxr, Coor wyb, Coor wyt)
{
    Coor x, y, line_x[4], line_y[4];
    Comment *element;

    if (CHECK_FLAG || ggTestColor (lay, "black")) return;

    if (!vis_arr[v_label]) return; /* labels not visible */

    ggSetColor (lay);
    d_fillst (fillst[lay]);

    for (element = firstElement; element; element = element -> next) {
	if (element -> type == GA_LABEL && LABELOBJ.maskno == lay) {
	    x = LABELOBJ.x;
	    if (x > wxr || x+1 < wxl) continue;
	    y = LABELOBJ.y;
	    if (y > wyt || y+1 < wyb) continue;
	    line_x[0] = line_x[1] = line_x[2] = line_x[3] = x;
	    ++line_x[3];
	    line_y[0] = line_y[1] = line_y[2] = line_y[3] = y;
	    ++line_y[1]; ++line_y[2];
