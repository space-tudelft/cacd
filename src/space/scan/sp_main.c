...
#ifdef DISPLAY
extern bool_t goptDrawDelaunay;
#endif
...
int sp_main (int argc, char **argv)
{
...
#ifdef CONFIG_XSPACE
    optDisplay = TRUE;
    initXpre (&argc, argv); // X options must be read before reading other options.
#endif
    ...
    *s++ = 'X';
#endif
    strcpy (s, "nuhvitx"); s += 7;
    t = s; /* remember end */
    strcpy (s, "a:D:E:e:P:p:S:");

    if (argc > 1 && argv[1][0] == '-' && argv[1][1] == '%') {
	++optSpecial;
	if (argv[1][2] == '%') ++optSpecial;
	s = optspecial;
	strcpy (s, "%L:"); s += 3;
	*s++ = 'f'; /* add fine network option */
#ifdef MOMENTS
	*s++ = 'g'; /* add moments option */
#endif
#ifdef MONITOR
	*s++ = 'm'; /* add monitor option */
#endif
	strcpy (s, "qwZ02"); /* add other special options */
	...
	switch (c) {
	    ...
	    case 'k': optIntRes  = TRUE; optPrick = TRUE; break;
	    case 'j': optIntRes  = TRUE; optInvertPrick = TRUE; break;
	    case 'G': optSelectiveElimination = TRUE; break;
	    ...
	    case 'X': optDisplay = optNoMenus = TRUE; break;
	    case '3': optCap3D = TRUE;      break;
	    ...
	    case 'g': optExtractMomentsInit = TRUE; break;
    ...
    for (argc = c = optind; argv[c]; ++c) if (*argv[c]) argv[argc++] = argv[c];
    argv[argc] = NULL;
	...
	say ("Use only one of -j and -k options."); optErr++;
    }

    if (optCap3D && !optCap) {
	say ("Use -c or -C or -l with -3 option."); optErr++;
    }
    ...
    if (!(optIntRes && optCap)) {
	    ...
	    say ("Use (-r or -z) and (-c or -C or -l) with -G option."); optErr++;
        }
        else if (optExtractMomentsInit) {
	    say ("Use (-r or -z) and (-c or -C or -l) with -g option."); optErr++;
        }
    }
#endif
...
#ifndef CONFIG_XSPACE
    if (!cellNames[0]) {
	say ("You must specify one or more cell names."); optErr++;
    }
#endif
	...
	fprintf (stderr, "\nUsage: %s ", argv0);
	if (optSpecial) {
	    fprintf (stderr, "[-%%%s] [-%%L max_depth]", optspecial+3);
	    fprintf (stderr, "\n       ");
	}
	*t = 0; /* set end */
	fprintf (stderr, "[-%s] [-a time] [-D min_depth]\n", optstring);
	fprintf (stderr, "       [-E file | -e def] [-P file | -p def] [-S param=value]");
#ifdef CONFIG_XSPACE
	fprintf (stderr, " [cell]\n\n");
#else
	fprintf (stderr, " cell ...\n\n");
	...
	optSubResMakefem  = paramLookupB ("sub3d.makefem", "off");
    }
#endif
    if (!optAlarmInterval) optAlarmInterval = paramLookupI ("progress_timer", "0");
    if (!optTorPos) optTorPos = paramLookupB ("component_coordinates", "off");
    if (!optBackInfo) optBackInfo = paramLookupI ("backannotation", "0");
    ...
    if (!optFlat) optFlat = paramLookupB ("flat_extraction", "off");
    if (!optFlat) optPseudoHier = paramLookupB ("expand_connectivity", "off");

    if (paramLookupB ("no_parasitics", "off")) {
        optCap3D = optLatCap = optCoupCap = optCap = FALSE;
        optIntRes = optAllRes = optResMesh = FALSE;
	optPrick = optInvertPrick = FALSE;
    ...
    if (!optFlat) {
	if ((optDisplay && !optPseudoHier)
#ifdef CAP3D
	 || (optCap3D && !paramLookupB ("allow_hierarchical_cap3d", "off"))
#endif
	 || (substrRes && !paramLookupB ("allow_hierarchical_subres", "off"))) {
	    optFlat = TRUE; optPseudoHier = FALSE;
	    if (optVerbose) message ("Flat extraction mode turned on!");
	}
    }

#ifdef CONFIG_XSPACE
    optNoMenus = !paramLookupB ("disp.show_menu", (optNoMenus ? "off" : "on"));
    initX (optind, argv, optstring);
#else
#ifdef DISPLAY
    if (optDisplay && optSpecial < 2) {
	s = cellNames[1];
	cellNames[1] = NULL;
	i = 1;
	while (cellNames[0]) {
	...
	/* Int == 0: default: only progress message by alarm from keyboard */
	/*	> 0: set alarm timer method (give alarm each +Int seconds) */
	/*	< 0: use alternative method (give alarm each -Int seconds) */
	if (setAlarmInterval (optAlarmInterval, catchAlarm)) /* no alarm signal available */
	    optAlarmInterval = -optAlarmInterval; /* use alternative method */
    }
#endif
    ...
    else if (add_sub_caps > 1 && optSubRes && optSubResInternal) {
	say ("warning: not adding substrate capacitances, not supported in internal mode");
	add_sub_caps = 0;
    }
...
#ifdef CONFIG_XSPACE
    interactive (dmproject, cellNames);
#else
#ifdef DISPLAY
    if (optDisplay) interactive (dmproject, cellNames);
#endif

    for (i = 0; cellNames[i]; ++i) extractTree (cellNames[i]);
...
#ifdef CAP3D
    if (optCap3D && optEstimate3D) cap3dEstimate (stdout);
#endif
    ...
    if (optPrintUnused) printUnusedParams ();
...
Private void optionImplications ()
{
    ...
    if (optSimpleSubRes && optSubRes) {
        ...
        say ("Only the 3D method is used");
	optSimpleSubRes = FALSE;
    }
    substrRes = optSimpleSubRes || optSubRes;
    optRes = optIntRes || substrRes;

    if (!optOnlyLastPass) alsoPrePass = substrRes || (optResMesh && !skipPrePass0) || optPrick;

#ifdef MOMENTS
#ifdef SNE
    if (!(optRes && optCap)) optExtractMoments = optSelectiveElimination = FALSE;
    else optExtractMoments = optSelectiveElimination ? TRUE : optExtractMomentsInit;
#endif
#endif

    optCapSave = optCap;
    optCoupCapSave = optCoupCap;
    optCap3DSave = optCap3D;
}
...
Private void extractTree (char *cellName)
{
	...
	Debug (fprintf (stderr, "%s, status %#o\n", obj -> name, obj -> status));
	     ...
	     if (obj -> status & ISLIBRARY) verbose ("%s skipped, has library status\n", obj -> name);
	else if (obj -> status & DEVMOD   ) verbose ("%s skipped, has device status\n", obj -> name);
	else if (obj -> status & ISMACRO  ) verbose ("%s skipped, has macro status\n", obj -> name);
	else if (obj -> status & DO_SPACE) {

	    if (obj -> status & (DO_EXP | DO_RESIZE)) {
		ASSERT (!optNoPrepro);
		expand (obj);
		obj -> status &= ~(DO_EXP | DO_RESIZE);
	    }
		...
		if (optResMesh && !skipPrePass0) {
		    lastPass = optOnlyPrePass && !optPrick && !substrRes;
		    substrRes = 0;
#ifdef DEBUG
		...
		if (substrRes) {
		    lastPass = optOnlyPrePass && !optPrick && (optSimpleSubRes || !optSubResInternal);
#ifdef DEBUG
		    ...
		    if (optSimpleSubRes) {
			processgln (obj -> name); /* run makedela */
		    }
		    else { /* optSubRes */
			if (optSubResInternal) {
			    ...
			    optResMesh = 0;
			    pp1SetColors ();
			    prePass1 = 2; prepass (obj); prePass1 = 0;
			    pp1ResetColors ();
			    optResMesh = optRM;
#ifdef DEBUG
			    if(DEBUG) fprintf(stderr, "PASS: finished prepass 1B\n");
#endif
			}
			else if (!(optDisplay && optOnlyPrePass)) {
			    processgln (obj -> name); /* run makesubres */
			}
		    }
#ifdef DEBUG
		...
		if (optPrick) { /* prePass2 is only for selective res, see lump! */
		    lastPass = optOnlyPrePass;
		    substrRes = 0;
#ifdef DEBUG
		    if(DEBUG) fprintf(stderr, "PASS: starting prepass 2\n");
#endif
		    prePass = 2; prepass (obj); prePass = 0;
#ifdef DEBUG
		    if(DEBUG) fprintf(stderr, "PASS: finished prepass 2\n");
#endif
		    substrRes = optSimpleSubRes || optSubRes;
		}
	    }
		...
		if (optCap || optRes) optReduc = !optNoReduc;
		if (min_coup_cap > 0 || min_coup_area > 0) optReduc2 = TRUE;
#ifdef DEBUG
		...
		optReduc = optReduc2 = FALSE;
	    }
	}
	else {
	    char * type = "";
	    if (obj -> status & DONE_FLAT) {
		type = " flat";
	    }
	    else if (obj -> status & DONE_PSEUDO) {
...
Private void prepass (do_t * dObject)
{
    ...
    bool_t optBackInfoSave = optBackInfo;
    ...
    optCap = optCoupCap = optLatCap = optCap3D = FALSE;
    optBackInfo = FALSE;

    if (prePass1 && optSubRes) {
	optCap3D = TRUE;
	if (prePass1 != 2) { /* prepare pass */
    ...
    optBackInfo = optBackInfoSave;
    optRes = optResSave;
    optCap = optCapSave;
    optCoupCap = optCoupCapSave;
    optLatCap = optLatCapSave;
    optCap3D = optCap3DSave;
}
...
Private void extract (do_t * dObject)
{
	...
	if (prePass == 1) {
	    ...
	    else if (prePass1 == 2) { mode = "p1b";
		note = "for substrate resistance calculate"; }
	    else
		note = "for substrate resistance";
	}
	else { /* prePass2 */
	    mode = "p2";
	    note = "for selective extraction";
	}
...
#ifdef CAP3D
	if (optCap3D || optSubRes || optCap3DSave) {
	    coor_t width3d = cap3dInitParam ();
	    if (coor_sub_ext > 0 && (bbxr - bbxl + 2 * coor_sub_ext) > width3d) {
		/* The extension for the substrate is chosen such that it is a
		   multiple of the 3D bandwidth.  This is done because otherwise
		   we may have problems that the band borders will split off
		   very narrow tiles.
		*/
		coor_sub_ext = width3d * (1 + ((coor_sub_ext - 1) / width3d));
	    }
	}
#endif
...
#ifdef DISPLAY
	if (optDisplay) {
	    displayMode (mprintf ("c %s", cellname));
	    displayMode (mprintf ("s %d %d %g", inScale, outScale, layoutKey -> dmproject -> lambda));
	    if (vdimZtop) displayMode (mprintf ("z %g", vdimZtop));
	    worldCoordinates ((float)bbxl, (float)bbxr, (float)bbyb, (float)bbyt);
	}
#endif
	firstPass = 0;
    }
...
#ifdef DISPLAY
    if (optDisplay && prePass != 1) displayMode (mode);
#endif
    ...
    if (prePass1 != 2) initHierNames ();

    initExtract (circuitKey, layoutKey);

    if (prePass1 != 2) readTid (layoutKey, circuitKey);

#ifdef CAP3D
    if (optCap3D) { /* (re-)initialize cap3d module */
	bandWidth = cap3dInit (layoutKey);
	if (optLatCap && bandWidth < effectDist && extrPass) {
	    effectDist = bandWidth;
	    say ("warning: lat_cap_window reduced due to cap3d.be_window");
	}
    }
#endif
...
#ifdef DISPLAY
    if (goptDrawDelaunay && optSimpleSubRes && extrPass) drawDelaunay (layoutKey);
#endif

    closeInput ();

#ifdef CAP3D
    if (optCap3D) cap3dEnd ();
#endif

    endExtract ();

    if (prePass1 != 2) endHierNames ();

    if (prePass1 != 2) disposeTid ();
...
Private void expand (do_t *dObject)
{
	...
	verbose ("preprocessing %s (phase 1%s)\n", cellname, optFlat ? " - flattening layout" : "");
	run ("makeboxl", cellname, optFlat ? NULL : (optPseudoHier ? "-h" : "-H"));
...
Private void run (char *command, char *cellname, char *option)
{
 ...
 // fprintf (stderr, "%s %s %s\n", path, option, cellname ? cellname : "");
	...
	say ("error in '%s %s %s'", path, option, cellname ? cellname : "");
	die ();
    }
}
...
int matherr (struct exception *x)
{
    ...
    switch (x -> type) {
	case DOMAIN:
	    fprintf (stderr, "argument domain error\n");
	    break;
	case SING:
	    fprintf (stderr, "argument singularity\n");
	    break;
	case OVERFLOW:
	    fprintf (stderr, "overflow range error\n");
	    break;
	case UNDERFLOW:
	    fprintf (stderr, "underflow range error\n");
	    break;
	case TLOSS:
	    fprintf (stderr, "total loss of significance\n");
	    break;
	case PLOSS:
	    fprintf (stderr, "partial loss of significance\n");
	    break;
	default:
