...
Private void newTileRef (tile_t *tile, subnode_t *sn)
{
    group_t *g;
    contRef_t *c;

    g = Grp (sn -> node);
    if (g -> tileInfo -> conts) return;

    g -> tileInfo -> conts = c = NEW (contRef_t, 1);
    if (!selResTiles) selResTiles = c;
    else selResTilesLast -> next = c;

    c -> nr = tile -> cnt;
    c -> cx = sn -> cond -> con;
    c -> xl = tile -> xl;
    c -> bl = tile -> bl;
    c -> prev = selResTilesLast;
    c -> next = NULL;
    c -> cnext = NULL;
    selResTilesLast = c;
}
...
void enumPair (tile_t *tile, tile_t *newerTile, int edgeOrien)
{
...
#ifdef DISPLAY
    if (goptPairBoundary && prePass != 1) drawPairBoundary (bdr -> x1, bdr -> y1, bdr -> x2, bdr -> y2, cx);
#endif
    if (!cx) return;
    ...
    if (optRes) { resEnumPair (tile, newerTile, edgeOrien); return; }
	...
	if (newtile) {
		...
		switch (el -> type) {
		    case RESELEM:
			res = &el -> s.res;
			if ((cx = res -> con) >= nrOfCondStd) break;
...
#ifdef CAP3D
    if (optCap3D) spiderPair (tile, newerTile, edgeOrien);
#endif
}
...
Private void pp1EnumPair (tile_t *tile, tile_t *newerTile, int edgeOrien)
{
...
#ifdef CAP3D
    if (prePass1 == 2) { /* pp1bEnumPair */ /* optSubRes */
	...
	if (tsc && nsc) {
		...
		subContJoin (tile, newerTile); i = 0;
	    }
	    else subContGroupJoin (tsc -> subcontInfo, nsc -> subcontInfo);
	}
#ifdef DISPLAY
	if (goptDrawSubContact && i) drawPairBoundary (bdr -> x1, bdr -> y1, bdr -> x2, bdr -> y2, 2);
#endif
...
do_it2:
	    ...
	    if (nsc && (optSubResSave || optSubRes) && (cx = nsc -> causing_con) >= 0) {
...
#endif
#ifdef CAP3D
	if (optSubRes) spiderPair (tile, newerTile, edgeOrien);
#endif
    }
}
...
Private void pp0EnumPair (tile_t *tile, tile_t *newerTile, int edgeOrien) /* prePass == 1 */
{
	...
	for (; el && el -> type == RESELEM; el = elem[++i]) {
	    res = &el -> s.res;
	    if ((cx = res -> con) < nrOfCondStd && !newerTile -> cons[cx]) {
		newerTile -> cons[cx] = sn = CONS (newerTile, cx);
		sn -> cond = res; // ??
		sn -> highres = 0;
		if (res -> val > 0) {
...
Private void pp2EnumPair (tile_t *tile, tile_t *newerTile, int edgeOrien) /* prePass */
{
    ...
    if (prePass == 1 && bandWidth2) { pp0EnumPair (tile, newerTile, edgeOrien); return; }
	...
	if (newtile) {
		...
		if ((cx = res -> con) < nrOfCondStd && !newerTile -> cons[cx]) {
		    newerTile -> cons[cx] = sn = CONS (newerTile, cx);
		    if (prePass == 2) { /* prePass2 */
			sn -> cond = res;
			st = NULL;
			if ((bdr_length && (st = tile -> cons[cx])) || (st = newerTile -> stb -> cons[cx])) {
			    if (st -> cond -> type != sn -> cond -> type) {
				st = newerTile -> stb -> cons[cx];
				if (st && st -> cond -> type != sn -> cond -> type) st = NULL;
			    }
			}
			subnodeCopy2 (st, sn);
			if (++cx > nHasConduc) nHasConduc = cx;
			if (res -> val > 0) newTileRef (newerTile, sn);
		    }
		    else if (res -> val > 0) { /* prePass0 */
	    ...
	    if (nHasConduc) { /* prePass2 -- read the contact elements */
		newerTile -> known = nHasConduc;
		if (!tile -> known || !bdr_length) nHasConduc = 0;
		while (el && el -> type != CONTELEM) el = elem[++i];
		while (el && el -> type == CONTELEM) {
		    if ((joiningCon = el -> s.cont.con1) < nrOfCondStd) {
			if (el -> s.cont.val > 0 && (sn = newerTile -> cons[joiningCon])) {
			    newTileRef (newerTile, sn);
			    cx = el -> s.cont.con2;
			    if (cx >= 0 && cx < nrOfCondStd && (st = newerTile -> cons[cx])) {
				newTileRef (newerTile, st);
				subnodeJoin2 (sn, st); /* don't join substrate contacts */
			    }
			}
		    }
		    el = elem[++i];
	...
	else if (prePass == 2 && tile -> known && newerTile -> known && bdr_length) {
	    nHasConduc = tile -> known < newerTile -> known ? tile -> known : newerTile -> known;
	    for (joiningCon = 0; joiningCon < nHasConduc; ++joiningCon) {
		if ((st = tile -> cons[joiningCon]) && (sn = newerTile -> cons[joiningCon])) {
		    if (st -> cond -> type == sn -> cond -> type) subnodeJoin2 (st, sn);
		}
	    }
	}
    }
	...
	if (newLookTerm (bdr -> x1, bdr -> y1)) {
		...
		else if (resPresent && term -> y > bdr -> y1) {
		    bdr -> y2 = term -> y;
    ...
    if (hasEdgeConnect && nHasConduc) { /* prePass2 */
	int j = 0;
again:
	elem = recogEdge (newerTile, tile, NULL);
	el = elem[i = 0];
	while (el && el -> type != PNCONELEM) el = elem[++i];
	while (el && el -> type == PNCONELEM) {
	    pnc = &el -> s.pnc;
	    if (pnc -> con1 < nrOfCondStd) {
		if ((st = tile -> cons[pnc -> con1])) /* Edge */
		if ((sn = newerTile -> cons[pnc -> con2]))
		if (st -> cond -> type == sn -> cond -> type) subnodeJoin2 (st, sn);
	    }
	    el = elem[++i];
	}
	if (++j < 2) { Swap (tile_t*, newerTile, tile); goto again; }
    }
}

Private void
resEnumPair (tile_t *tile, tile_t *newerTile, int edgeOrien)
{
    int i, cx, fromNewer, resPresent;
...
do_it2:
	    ...
	    if (resPresent) {
		contRef_t *c;
		while (selResTiles && selResTiles -> nr == tileCnt) { /* optPrick */
		    selResTiles = (c = selResTiles) -> next;
		    if ((sn = newerTile -> cons[c -> cx])) sn -> highres = 1;
...
#define TOR_AND_DSCON(t1,t2) t1->tor && (sn = t2->cons[t1->tor->type->s.tor.dsCon]) && (sn->highres & 1)
	...
	if (bdr_length) {
		...
		if (TOR_AND_DSCON (tile, newerTile)) SET_conLR2 (newerTile);
		if (TOR_AND_DSCON (newerTile, tile)) SET_conLR1 (tile);
	    }
	}
	else cx = 0;
	...
	if (cx == 2) { SET_conTB1 (tile); SET_conTB2 (newerTile); }
	    ...
	    if (TOR_AND_DSCON (tile, newerTile)) SET_conTB2 (newerTile);
	    if (TOR_AND_DSCON (newerTile, tile)) SET_conTB1 (tile);
	}
...
connectPoints (tile_t *tile, tile_t *otherTile, int edgeOrien, coor_t x, coor_t y)
{
    ...
    subnode_t *psn, *sn, *othersn, **ocons, **tcons;
    int i, j, n, both_eq_conductor;
	...
	if (ocons[i]) {
		...
		psn = prevPoint -> cons[i];
		if (!(ocons[i] -> highres & 1)) {
		    subnodeCopy (psn, othersn); goto ready1;
		}
		subnodeNew2 (othersn, Grp (psn -> node));
		if (psn -> pn) polnodeCopy (psn, othersn);
		goto ready0;
	    }
	    if (prevPoint && (psn = prevPoint -> cons[i])) {
		if (othersn -> cond -> type == psn -> cond -> type) {
		    subnodeCopy (psn, othersn); goto ready1;
		}
		warnSubnodeJoin (i, otherPoint -> x, otherPoint -> y, 1);
	    }
...
ready1:
		...
		psn = ocons[i];
		if (psn -> cond -> val > 0) { /* high res */
		    j = 1;
		    if (optPrick) {
			group_t *g = Grp (othersn -> node);
			if (psn -> highres) g -> prick = 1;
			if (!psn -> cond -> rKeep) {
			    if (optInvertPrick) { if (g -> prick) j = 0; }
			    else if (!g -> prick) j = 0;
			}
		    }
		...
		else if (psn -> highres) { /* optPrick: contact */
		    Grp (othersn -> node) -> prick = 1;
		    psn -> highres = 0;
		}
	    }
	    if (sn && both_eq_conductor) {
		if (edgeOrien != 'h') { /* v */
...
Private void joinPoints (tile_t *tile, tile_t *otherTile, int edgeOrien)
{
    nodePoint_t *point, *otherPoint;
    subnode_t *sn, *osn;

    if (edgeOrien == 'v') {
	point = tile -> rbPoints; otherPoint = otherTile -> tlPoints;
    } else {
	point = tile -> tlPoints; otherPoint = otherTile -> rbPoints;
    }

    for (joiningCon = 0; joiningCon < nrOfCondStd; ++joiningCon) {
	if ((sn = point -> cons[joiningCon]) && (osn = otherPoint -> cons[joiningCon])) {
	    if (osn -> cond -> type == sn -> cond -> type) {
		subnodeJoin (sn, osn);
		sn  = tile -> cons[joiningCon];
		osn = otherTile -> cons[joiningCon];
		if (edgeOrien == 'v') {
		    sn  -> highres |= 0x10; /* LR1 */
		...
		if (osn -> highres & 1) { otherPoint -> fixed = 1;
		     if (sn -> highres & 1) point -> fixed = both_same_highres = 1; }
		else if (sn -> highres & 1) point -> fixed = 1;
	    }
	    else warnSubnodeJoin (joiningCon, otherPoint -> x, otherPoint -> y, 1);
	}
    }
}
...
updateEdgeCap (elemDef_t **elem, tile_t *tile, tile_t *adjTile)
{
 ...
 /* ASSERT (extrPass && !optRes && !substrRes); */
	...
	if (elem[i] -> type == EDGECAPELEM) {
	    ...
	    if (cap -> pKeep) subnA -> node -> term = 2;
		...
		if (!subnB) missingCon (cap -> nMask, cap -> nOccurrence, /* die */
				tile, adjTile, tNull, elem[i], bdr -> x1, bdr -> y1);
		if (cap -> nKeep) subnB -> node -> term = 2;
	    }
	...
	nodePoint_t *pointStart, nodePoint_t *adjPointStart, int use_ttl)
{
    ...
    double cap;
    int i, cx, cy, assign_type, typeA, typeB;
    int j, h = 0;
...
snA:
	    subnA2 = q1 -> cons[cx];
	    if (ced -> pKeep) subnA1 -> node -> term = subnA2 -> node -> term = 2;
		...
		if (substrRes) {
		    ...
		    if (ced -> nKeep) subnB1 -> node -> term = 2;
		}
		subnB2 = subnB1;
		assign_type = 0;
	    }
		...
		if (!subnB1) missingCon (ced -> nMask, ced -> nOccurrence, /* die */
			tile, adjTile, tNull, elem[i], bdr -> x1, bdr -> y1);
		subnB2 = q2 -> cons[cy];
		if (ced -> nKeep) subnB1 -> node -> term = subnB2 -> node -> term = 2;
		assign_type = cap_assign_type;
	    }

	    cap = ced -> val * bdr_length;

	    if (assign_type & 1) {
		capAdd (subnA1, subnB1, cap / 3, ced -> sortNr);
		capAdd (subnA2, subnB2, cap / 3, ced -> sortNr);
		capAdd (subnA1, subnB2, cap / 6, ced -> sortNr);
		capAdd (subnA2, subnB1, cap / 6, ced -> sortNr);
	    }
	    else {
		capAdd (subnA1, subnB1, cap / 2, ced -> sortNr);
		capAdd (subnA2, subnB2, cap / 2, ced -> sortNr);
	    }
	}
	else if (elem[i] -> type == LATCAPELEM) return i;
    }
