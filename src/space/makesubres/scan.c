...
void scan (edge_t *newEdge)
{
    ...
    coor_t nextX, nextStrip;
    int stripSplit, doSplit;

    scanInfo.edges = 0;
    scanInfo.srl   = 0;
    scanInfo.xpos  = 0;

    if ((thisX = newEdge -> xl) == INF) goto ret;
	...
	while (edge -> yr < INF || newEdge -> xl == thisX) {
	    ...
	    */
		...
		if (edge -> xl == thisX) {
			...
			TestIntersection (edge, edge -> bwd);
		    tileCrossEdge (edge, (stripSplit || doSplit));
		}
    ...
    while (nextStrip != INF) nextStrip = sub3dStrip ();
    sub3dStop ();
    tileStopScan ();
