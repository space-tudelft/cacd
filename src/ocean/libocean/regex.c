...
/* These are the command codes that appear in compiled regular
   expressions.  Some opcodes are followed by argument bytes.
   A command code can specify any interpretation whatsoever for its
...
#ifdef DEBUG
static void extract_number_and_incr (int *dest, unsigned char **source)
{
...
/* Set by `re_set_syntax' to the current regexp syntax to recognize.
   Can also be assigned to arbitrarily: each pattern buffer stores its
   own syntax, so it can be changed between regex compilations. */
...
static reg_errcode_t regex_compile (const char *pattern,
	int size, reg_syntax_t syntax, struct re_pattern_buffer *bufp)
{
	...
	case '|':
        ...
        case '\\':
	      ...
	      if (COMPILE_STACK_EMPTY) {
              ...
              /* Since we just checked for an empty stack above, this ``can't happen''. */
...
#define PUSH_PATTERN_OP(pattern_op, fail_stack)				\
  ((FAIL_STACK_FULL () && !DOUBLE_FAIL_STACK (fail_stack))		\
    ? 0 : ((fail_stack).stack[(fail_stack).avail++] = pattern_op, 1))
...
void re_set_registers (struct re_pattern_buffer *bufp,
    struct re_registers *regs, unsigned num_regs, regoff_t *starts, regoff_t *ends)
{
  if (num_regs)
    {
          ...
          if (regs && !bufp->no_sub)
	    {
              ...
              if (bufp->regs_allocated == REGS_UNALLOCATED)
                { /* No.  So allocate them with malloc.  We need one
                     ...
                     GNU code uses.  */
		  regs->num_regs = RE_NREGS > num_regs ? RE_NREGS : num_regs + 1;
		  if (!(regs->start = TALLOC (regs->num_regs, regoff_t))) return -2;
		  if (!(regs->end   = TALLOC (regs->num_regs, regoff_t))) return -2;
                  bufp->regs_allocated = REGS_REALLOCATE;
                }
                  ...
                  if (regs->num_regs < num_regs + 1)
                    {
                      ...
                      if (!regs->start || !regs->end) return -2;
                    }
                }
              else
	      ...
	      num = regs->num_regs;
	      if (num_regs < num) num = num_regs;

	      for (mcnt = 1; mcnt < num; mcnt++)
		{
              ...
              /* If the regs structure we return has more elements than
                 ...
                 If we (re)allocated the registers, this is the case,
...
{
  const char *msg = !re_error_msg[errcode] ? "Success" : re_error_msg[errcode];
...
{
  if (preg->buffer) free (preg->buffer);
  preg->buffer = NULL;

  preg->allocated = 0;
  preg->used = 0;

  if (preg->fastmap) free (preg->fastmap);
  preg->fastmap = NULL;
  preg->fastmap_accurate = 0;

  if (preg->translate) free (preg->translate);
  preg->translate = NULL;
}
