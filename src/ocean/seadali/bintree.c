...
void to_bintree (struct bintree *p, Coor val)
{
    ...
    if (val <p -> value) {
	if (!p -> left) p -> left = mk_bintree (val);
	else to_bintree (p -> left, val);
	return;
    }
    if (!p -> right) p -> right = mk_bintree (val);
    else to_bintree (p -> right, val);
}
...
void to_edgetree (struct edgetree *p, struct x_lst *edge)
{
	...
	if (!p -> left) p -> left = mk_edgetree (edge);
	else to_edgetree (p -> left, edge);
	return;
    }
    if (!p -> right) p -> right = mk_edgetree (edge);
    else to_edgetree (p -> right, edge);
}
...
static void lnk_edgetree (struct edgetree *p)
{
    ...
    if (!edgeprev) edgehead = p -> edge;
    else edgeprev -> next = p -> edge;
