...
static void add_wire_part (Coor x1, Coor y1, Coor x2, Coor y2, short left, short right);
...
void add_wire (Coor stack_x[], Coor stack_y[], int npoints, Coor width, int non_orth, int wire_ext)
{
    ...
    int  sxp = 0, syp = 0;
    Coor w = width / 2;	/* w == width from center of wire to surroundings of wire */
    Coor nw = (Coor) ((double)w / sqrt (2.0) + 1.0);
    Coor lw;			/* length of wire */

    /* extension of wires at the start and stop coordinates */
    if (wire_ext) ext1 = w;
    else if (!non_orth) ext1 = ((width / QUAD_LAMBDA) % 2 == 0) ? 0 : QUAD_LAMBDA / 2;
