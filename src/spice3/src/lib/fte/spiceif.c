...
static IFvalue *doask(char *ckt, int typecode, GENinstance *dev, GENmodel *mod, IFparm *opt, int ind);
static int doset(char *ckt, GENinstance *dev, GENmodel *mod, IFparm *opt, struct dvec *val);
static int finddev(char *ck, char *name, GENERIC **devptr, GENERIC **modptr);
static IFparm *parmlookup(IFdevice *dev, GENinstance **inptr, char *param, int do_model, int inout);
...
char * if_inpdeck (struct line *deck, INPtables **tab)
{
    ...
    *tab = INPtabInit(i);
    ...
    if ((err = (*(ft_sim->newCircuit))(&ckt)) != OK) {
    ...
    for (j = 0; j < ft_sim->numAnalyses; j++) {
        if (eq(ft_sim->analyses[j]->name, "options")) {
            which = j;
            break;
        }
    }
    if (which != -1) {
        err = IFnewUid(ckt, &optUid, (IFuid)NULL, "options", UID_ANALYSIS, (GENERIC**)NULL);
        if (err) {
            ft_sperror(err, "newUid");
            return(NULL);
        }
        err = (*(ft_sim->newAnalysis))(ft_curckt->ci_ckt, which, optUid,
                (GENERIC**)&ft_curckt->ci_defOpt, (GENERIC*)ft_curckt->ci_defTask);
        if (err) {
...
int if_run (char *t, char *what, wordlist *args, char *tab)
{
	...
	eq(what, "noise")) {
        ...
        err = IFnewUid(ft_curckt->ci_ckt, &specUid, (IFuid)NULL, "special", UID_TASK, (GENERIC**)NULL);
        if (err) {
            ft_sperror(err, "newUid");
            return(2);
        }
        err = (*(ft_sim->newTask))(ft_curckt->ci_ckt, (GENERIC**)&ft_curckt->ci_specTask, specUid);
        if (err) {
            ft_sperror(err, "newTask");
            return(2);
        }
        ...
        if (which != -1) {
            err = IFnewUid(ft_curckt->ci_ckt, &optUid, (IFuid)NULL, "options", UID_ANALYSIS, (GENERIC**)NULL);
            if (err) {
                ft_sperror(err, "newUid");
                return(2);
            }
            err = (*(ft_sim->newAnalysis))(ft_curckt->ci_ckt, which, optUid,
                    (GENERIC**)&ft_curckt->ci_specOpt, (GENERIC*)ft_curckt->ci_specTask);
            if (err) {
                ft_sperror(err, "createOptions");
                return(2);
            }
...
int if_option(char *ckt, char *name, int type, char *value)
{
    ...
    for (i = 0; i < ft_sim->numAnalyses; i++) {
        if (eq(ft_sim->analyses[i]->name, "options")) {
            which = i;
            break;
        }
    }
    ...
    switch (ft_sim->analyses[which]->analysisParms[i].dataType & IF_VARTYPES) {
        ...
        case IF_FLAG:
            ...
            break;
        default:
            fprintf(cp_err, "if_option: Internal Error: bad option type %d.\n",
...
void if_dump(char *ckt, FILE *file)
{
 ...
 */
struct variable * spif_getparam(char *ckt, char **name, char *param, int ind, int do_model)
{
    ...
    if (param && eq(param, "all")) {
        ...
        pv = doask(ckt, typecode, dev, mod, opt, ind);
        if (pv) vv = parmtovar(pv, opt);
        return (vv);
    } else
        return (if_getstat(ckt, *name));
}

void if_setparam (char *ckt, char **name, char *param, struct dvec *val, int do_model)
{
    IFparm *opt;
    IFdevice *device;
    GENmodel *mod = (GENmodel *)NULL;
...
static struct variable *parmtovar(IFvalue *pv, IFparm *opt)
{
    ...
    switch (opt->dataType & IF_VARTYPES) {
        ...
        case IF_REALVEC:
            ...
            break;
        default:
            fprintf(cp_err, "parmtovar: Internal Error: bad PARM type %d.\n", opt->dataType);
            return (NULL);
    }
...
static IFvalue *doask(char *ckt, int typecode, GENinstance *dev, GENmodel *mod, IFparm *opt, int ind)
{
    ...
    if (dev)
        err = (*(ft_sim->askInstanceQuest))((GENERIC*)ckt, (GENERIC*)dev, opt->id, &pv, (IFvalue*)NULL);
    else
        err = (*(ft_sim->askModelQuest))((GENERIC*)ckt, (GENERIC*) mod, opt->id, &pv, (IFvalue*)NULL);
    if (err != OK) {
...
static int doset(char *ckt, GENinstance *dev, GENmodel *mod, IFparm *opt, struct dvec *val)
{
	...
	switch (opt->dataType & (IF_VARTYPES & ~IF_VECTOR)) {
	    ...
	    default:
		fprintf(cp_err, "Can't assign value to \"%s\" (unsupported vector type)\n", opt->keyword);
		return E_UNSUPP;
	}
    } else {
	switch (opt->dataType & IF_VARTYPES) {
	    ...
	    default:
		fprintf(cp_err, "Can't assign value to \"%s\" (unsupported type)\n", opt->keyword);
		return E_UNSUPP;
	}
    }

    /* fprintf(cp_err, "Calling doask(%x, %x, %x)\n", dev, mod, opt); */
...
void if_setndnames (char *line) /* BLOW THIS AWAY */
{
    char *t;
    int i;
    ...
    if (!(i = inp_numnodes(*t))) return;
    if ((*t == 'q') || (*t == 'Q')) i = 3;

    cp_addkword(CT_DEVNAMES, t);
    while (i-- > 0) {
        t = gettok(&line);
        if (t) cp_addkword(CT_NODENAMES, t);
    }
}
#endif
...
int if_analQbyName(GENERIC *ckt, int which, GENERIC *anal, char *name, IFvalue *parm)
{
    int i;
    for (i = 0; i < ft_sim->analyses[which]->numParms; i++) {
        if (eq(ft_sim->analyses[which]->analysisParms[i].keyword, name)) {
            return((*(ft_sim->askAnalysisQuest))(ckt, anal,
                    ft_sim->analyses[which]->analysisParms[i].id, parm, (IFvalue *)NULL));
        }
    }
    return(E_BADPARM);
}
...
bool if_tranparams(struct circ *ci, double *start, double *stop, double *step)
{
    ...
    for (i = 0; i < ft_sim->numAnalyses; i++) {
        if (eq(ft_sim->analyses[i]->name, "TRAN")) {
            which = i;
            break;
        }
    }
 ...
 * available.
 */
struct variable * if_getstat (char *ckt, char *name)
{
    int i;
    ...
    if (name) {
        ...
        if ((*(ft_sim->askAnalysisQuest))(ckt, ft_curckt->ci_curTask,
               ft_sim->analyses[which]->analysisParms[i].id, &parm, (IFvalue *)NULL) == -1) {
        ...
        for (i = 0, vars = v = NULL; i < ft_sim->analyses[which]->numParms; i++) {
            ...
            if (v) {
                v->va_next = parmtovar(&parm, &ft_sim->analyses[which]->analysisParms[i]);
                v = v->va_next;
            } else {
                vars = v = parmtovar(&parm, &ft_sim->analyses[which]->analysisParms[i]);
            }
        }
        return (vars);
    }
