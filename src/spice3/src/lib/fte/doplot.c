...
/* enable screen plot selection for these display types */
    ...
    if (!wl && hc_button) {
        ...
        if (response.option == error_option) goto ret;

	if (response.reply.graph) {

	    if (DevSwitch(devtype)) goto ret;
	...
	if (!wl) {
	    ...
	    if ((buf2 = prompt(cp_in)) == NULL) goto ret; /* prompt aborted */
	    wl = alloc(wordlist);
	    wl->wl_word = buf2;
	    wl->wl_next = NULL;
	    wl = process(wl);
	...
	ok = plotit(wl, fname, (char *) NULL);

	DevSwitch(NULL);

	if (tw) wl_free(tw);

	if (!ok) { printf("com_hardcopy: graph not defined\n"); goto ret; }
    }
	...
	if (!cp_getvar("lprplot5", VT_STRING, format))
        ...
        sprintf(buf, format, device, fname);
...
#endif
#ifdef SYSTEM_PSLPR
      if (!printed && eq(devtype, "postscript")) {
    ...
    if (!printed) {
      if (eq(devtype, "plot5")) {
        fprintf(cp_out, "The file \"%s\" may be printed with the Unix \"plot\" command,\n", fname);
        fprintf(cp_out, "\tor by using the '-g' flag to the Unix lpr command.\n");
      } else if (eq(devtype, "postscript")) {
        fprintf(cp_out, "The file \"%s\" may be printed on a postscript printer.\n", fname);
      }
    }
ret:
    if (tempf && *device) unlink(fname);
}
...
static bool plotit (wordlist *wl, char *hcopy, char *devname)
{
    ...
    sprintf(cline, "plot %s", (xn = wl_flatten(wwl))); free(xn);
    wl_free(wwl);

    /* Now extract all the parameters. */

    /* In case the parameter is the first on the line, we need wl_prev.
     */
    wl = wl->wl_prev;
    if (!wl) { fprintf(cp_err, "Error: no plot root\n"); return (false); }
    ...
    for (d = vecs; d; d = d->v_link2)
        if (((d->v_type == SV_POLE) || (d->v_type == SV_ZERO)) !=
                oneval ? 1 : 0) {
            fprintf(cp_err, "Error: plot must be either all pole-zero or contain no poles or zeros\n");
            return (false);
        }
	...
	for (d = vecs; d; d = d->v_link2) {
		...
		*prevvp = d;
	    ...
	    if (isreal(d)) {
		fprintf(cp_err, "Warning: plotting real data \"%s\" on a smith grid\n", d->v_name);
    ...
    if ((xlims[0] <= 0.0) && ((gtype == GRID_XLOG) || (gtype == GRID_LOGLOG))) {
        fprintf(cp_err, "Error: X values must be > 0 for log scale\n");
        return (false);
    }
    if ((ylims[0] <= 0.0) && ((gtype == GRID_YLOG) || (gtype == GRID_LOGLOG))) {
    ...
    if (devname && eq(devname, "lpr")) {
        ...
        if (!(!ft_curckt || !ft_curckt->ci_ckt ||
            !eq(ft_curckt->ci_name, plot_cur->pl_title) ||
          ...
          for (v = vecs; v; v= v->v_link2) {
            newdata = allocn(double, newlen);

            if (!ft_interpolate(v->v_realdata, newdata,
                v->v_scale->v_realdata, v->v_scale->v_length, newscale, newlen, 1)) {
...
static double * getlims (wordlist *wl, char *name, int number)
{
    ...
    for (beg = wl; beg; beg = beg->wl_next) {
        if (eq(beg->wl_word, name)) {
            if (beg == wl) {
                fprintf(cp_err, "Syntax error: looking for plot parameters \"%s\".\n", name);
                return (NULL);
            }
            wk = beg;
            if (number) {
                d = allocn(double, number);
                for (n = 0; n < number; n++) {
                    wk = wk->wl_next;
                    if (!wk) {
                        fprintf(cp_err, "Syntax error: not enough parameters for \"%s\".\n", name);
                        return (NULL);
                    }
                    ss = wk->wl_word;
    ...
    for (beg = wl; beg; beg = beg->wl_next) {
        if (eq(beg->wl_word, name)) {
            if (beg == wl || !beg->wl_next) {
                fprintf(cp_err, "Syntax error: looking for plot keyword at \"%s\".\n", name);
                return (NULL);
            }
            wl = beg->wl_next;
            s = wl->wl_word;
            wl->wl_word = NULL;
            beg->wl_prev->wl_next = wl->wl_next;
            if (wl->wl_next) {
		wl->wl_next->wl_prev = beg->wl_prev;
		wl->wl_next = NULL;
	    }
            wl_free(beg);
            return (s);
...
static void xtend (struct dvec *v, int length)
{
    ...
    if (isreal(v)) {
        od = v->v_realdata;
        v->v_realdata = allocn(double, length);
        ...
        oc = v->v_compdata;
        v->v_compdata = allocn(complex, length);
        for (i = 0; i < v->v_length; i++) {
...
static void compress (struct dvec *d, double *xcomp, double *xind)
{
    ...
    if (xind) {
        ilo = (int) xind[0];
        ihi = (int) xind[1];
        if (ilo <= ihi && ilo > 0 && ilo < d->v_length && ihi > 1 && ihi <= d->v_length) {
            newlen = ihi - ilo;
            dd = (double *) tmalloc(newlen * sz);
            cc = (complex *) dd;
            if (isreal(d)) {
                bcopy((char *) (d->v_realdata + ilo), (char *) dd, newlen * sz);
                tfree(d->v_realdata);
                d->v_realdata = dd;
            } else {
                bcopy((char *) (d->v_compdata + ilo), (char *) cc, newlen * sz);
                tfree(d->v_compdata);
                d->v_compdata = cc;
            }
            d->v_length = newlen;
