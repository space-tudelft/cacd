...
void wl_forall(wordlist *wl, void (*fn)(), char *data)
{
	while (wl) {
		(*fn)(wl, data);
		wl = wl->wl_next;
...
dgen * dgen_init(GENcircuit *ckt, wordlist *wl, int nomix, int flag, int model)
{
	...
	if (instance && model) {
		fprintf(stderr, "Error: can't mix instances and models");
		tfree(dg);
		return NULL;
	}
#endif
...
static void dgen_next(dgen **dgx)
{
	...
	while (!done) {
		...
		if (need & DGEN_INSTANCE && !dg->instance) continue;
		if (need & DGEN_MODEL && !dg->model) continue;
		...
		for (w = dg->dev_list; w && !done; w = w->wl_next) {
			...
			if (device && *device) {
				...
				if (!dev_name) {
					...
					continue;
				} else if (!eq(device, dev_name + 1 + subckt_len)) {
					done = 0;
					/*printf("Wrong name.\n");*/
					continue;
				}
			} else if (model && *model) {
				if (!eq(model, mod_name)) {
					done = 0;
					need |= DGEN_MODEL;
