...
void com_reshape (wordlist *wl)
{
	...
	for (w = wl; w; w = w->wl_next) {
	    if ((p = strchr(w->wl_word, '['))) break;
	}

	if (p) {
	    if (p != w->wl_word) w = w->wl_next;
	    wlast = w;
	    *p++ = 0;
	} else
	    wlast = NULL;
	...
	if (err || numdims > MAXDIMS) continue;

	/* Copy dimensions from the first item if none are explicitly given */
	if (!numdims) {
	    /* Copy from the first */
	    vname = cp_unquote((p = wl->wl_word));
	    dv = vec_get(vname);
	    if (!dv) printf("'%s' dimensions vector not found\n", vname);
	    if (vname != p) txfree(vname);
	    if (!dv) return;
	...
	prod = 1;
	for (i = 0; i < numdims; i++) prod *= dims[i];

	/* resize each vector */
	for (w2 = wl; w2 && w2 != w; w2 = w2->wl_next) {
	    vname = cp_unquote((p = w2->wl_word));

	    dv = vec_get(vname);
	    if (!dv) printf("'%s' vector not found\n", vname);
	    if (vname != p) txfree(vname);

	    /* The name may expand to several vectors */
	    for (d = dv; d; d = d->v_link2) {
		nprod = 1;
		for (i = 0; i < d->v_numdims; i++) nprod *= d->v_dims[i];
		if (nprod != d->v_length) {
