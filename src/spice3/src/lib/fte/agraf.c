 ...
 */
void ft_agraf(xlims, ylims, xscale, plot, vecs, xdel, ydel, xlog, ylog, nointerp)
    double *xlims, *ylims;
    ...
    bool xlog, ylog, nointerp;
{
    ...
    double xrange[2], yrange[2], x10, x1, x2, yy1, y2, x, y;
    int mag, hmt, lmt, dst, spacing, nsp, ypt, upper, lower, curline;
    double tenpowmag, diff, *values = NULL;
    struct dvec *v;
    ...
    if ((xscale->v_gridtype == GRID_YLOG) ||
        ...
        ylogscale = true;
    if (!cp_getvar("width",  VT_NUM, (char *) &maxy)) maxy = DEF_WIDTH;
    if (!cp_getvar("height", VT_NUM, (char *) &height)) height = DEF_HEIGHT;
    if (ft_nopage)
	nobreakp = true;
    else
	(void) cp_getvar("nobreak", VT_BOOL, (char *) &nobreakp);
    maxy -= (margin + FUDGE);
    ...
    field = allocn(char, (maxy + 1) * (maxx + 1));
    line1 = allocn(char, maxy + margin + FUDGE + 1);
    line2 = allocn(char, maxy + margin + FUDGE + 1);
    if (!novalue) values = allocn(double, maxx);
    ...
    if (xrange[0] > xrange[1] || yrange[0] > yrange[1]) {
        fprintf(cp_err, "ft_agraf: Internal Error: bad limits %lg, %lg, %lg, %lg\n",
    ...
    for (i = 0, j = lmt; j <= hmt; i += spacing, j += dst / nsp) {
        ...
        bcopy(buf, &line2[i + margin - ((j < 0) ? 2 : 1) - shift], strlen(buf));
    }
    ...
    if (!novalue)
        for (i = omargin + 1; i < margin - 2 && (vecs->v_name[i - omargin - 1]); i++)
     ...
     */
    x10 = 0;
    if (xrange[0] > 0.0 && xrange[1] > 0.0) x10 = xrange[1] / xrange[0];

    upper = lower = 0;
    for (i = 0; i < maxx; i++) {
        if (nointerp)
            x = isreal(xscale) ? xscale->v_realdata[i] : realpart(&xscale->v_compdata[i]);
        else if (xlog && x10 > 0.0)
	    x = xrange[0] * pow(10.0, mylog10(x10) * i / (maxx - 1));
        else
            x = xrange[0] + (xrange[1] - xrange[0]) * i / (maxx - 1);
        while ((isreal(xscale) ? (xscale->v_realdata[upper] < x) :
                (realpart(&xscale->v_compdata[upper]) < x)) && (upper < xscale->v_length - 1))
            upper++;
        while ((isreal(xscale) ? (xscale->v_realdata[lower] < x) :
                (realpart(&xscale->v_compdata[lower]) < x)) && (lower < xscale->v_length - 1))
            lower++;
        if ((isreal(xscale) ? (xscale->v_realdata[lower] > x) :
                (realpart(&xscale->v_compdata[lower]) > x)) && (lower > 0))
            lower--;
        x1 = (isreal(xscale) ? xscale->v_realdata[lower] : realpart(&xscale->v_compdata[lower]));
        x2 = (isreal(xscale) ? xscale->v_realdata[upper] : realpart(&xscale->v_compdata[upper]));
        if (x1 > x2) {
            fprintf(cp_err, "Error: X scale (%s) not monotonic\n", xscale->v_name);
            return;
        }
        for (v = vecs; v; v = v->v_link2) {
            yy1 = (isreal(v) ? v->v_realdata[lower] : realpart(&v->v_compdata[lower]));
            y2  = (isreal(v) ? v->v_realdata[upper] : realpart(&v->v_compdata[upper]));
            if (x1 == x2)
                y = yy1;
            else
    ...
    for (i = 0; i < omaxy + margin; i++) out_send("-");
    out_send("\n");
    i = (omaxy + margin - strlen(plot->pl_title)) / 2;
    while (i-- > 0) out_send(" ");
    strcpy(buf, plot->pl_title);
    buf[maxy + margin] = '\0';  /* Cut off if too wide */
    out_send(buf);
    out_send("\n");
    sprintf(buf, "%s %s", plot->pl_name, plot->pl_date);
    buf[maxy + margin] = '\0';
    ...
    if (j == 0) j = 1;
    for (v = vecs; v; v = v->v_link2) {
	sprintf(out_pbuf, "%c = %-17s", v->v_linestyle, v->v_name);
	out_send(out_pbuf);
        if (!(++i % j) && v->v_link2) {
            out_send("\n         ");
            curline++;
    ...
    out_send("\n");
    i = 0;
    out_send(line2); out_send("\n");
    out_send(line1); out_send("\n");
    curline += 2;
    for (i = 0; i < maxx; i++) {
        if (nointerp)
          x = isreal(xscale) ? xscale->v_realdata[i] : realpart(&xscale->v_compdata[i]);
