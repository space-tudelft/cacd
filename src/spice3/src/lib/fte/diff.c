...
static bool nameeq(char *n1, char *n2)
{
...
void com_diff(wordlist *wl)
{
    ...
    if (!eq(p1->pl_name, p2->pl_name))
        fprintf(cp_err, "Warning: plots %s and %s seem to be of different types\n",
            p1->pl_typename, p2->pl_typename);
    if (!eq(p1->pl_title, p2->pl_title))
        fprintf(cp_err, "Warning: plots %s and %s seem to be from different circuits\n",
            p1->pl_typename, p2->pl_typename);
        ...
        for (i = 0; i < j; i++) {
            if (v1->v_length <= i) {
                ...
                if (isreal(v1)) {
                    d1 = v1->v_realdata[i];
                    d2 = v2->v_realdata[i];
                    if (MAX(FABS(d1), FABS(d2)) * reltol + tol < FABS(d1 - d2)) {
                        fprintf(cp_out, "%s.%s[%d] = %-15s ",
			    p1->pl_typename, v1->v_name, i, printnum(d1));
                        fprintf(cp_out, "%s.%s[%d] = %s\n",
			    p2->pl_typename, v2->v_name, i, printnum(d2));
                    }
                } else {
                    c1 = v1->v_compdata[i];
                    c2 = v2->v_compdata[i];
                    ...
                    if (cmax * reltol + tol < cmag(&c3)) {
                        fprintf(cp_out, "%s.%s[%d] = %-10s, ",
			    p1->pl_typename, v1->v_name, i, printnum(realpart(&c1)));
                        fprintf(cp_out, "%-10s ", printnum(imagpart(&c1)));
                        fprintf(cp_out, "%s.%s[%d] = %-10s, ",
			    p2->pl_typename, v2->v_name, i, printnum(realpart(&c2)));
