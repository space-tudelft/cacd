...
void com_aspice (wordlist *wl)
{
	...
	if (!Spice_Path || !*Spice_Path) {
	    fprintf(cp_err, "No spice3 binary is available for the aspice command.\n");
	    return;
	}
        strcpy(spicepath, Spice_Path);
    }
    ...
    *t = '\0';
    ...
    fclose(inp);
    raw = smktemp("raw");
    fclose(fopen(raw, "w")); /* So there isn't a race condition. */
    pid = fork();
    if (pid == 0) {
...
"ft_checkkids: Internal Error: should be %d jobs done but there aren't any.\n", numchanged);
	...
	lp = NULL;
        ...
        while (fgets(buf, BSIZE_SP, fp)) out_send(buf);
        fclose(fp);
        if (!p->pr_saveout) unlink(p->pr_outfile);
        printf("\n-----\n");
    }
    printf("\n");
...
void com_rspice (wordlist *wl)
{
 ...
 /* int s; */
    ...
    if (*rhost == '\0') {
        fprintf(cp_err, "Error: there is no remote spice host for this site -- set \"rhost\".\n");
	return;
    }
    if (*program == '\0') {
        fprintf(cp_err, "Error: there is no remote spice program for this site -- set \"rspice\".\n");
	return;
    }
	...
	if (!strncmp("@@@", buf, 3)) {
		...
		fprintf(stderr, "Error adjusting rawfile: write \"%d\" at %ld\n", num, pos);
	    else
		fprintf(out, "%d", num);
	} else
