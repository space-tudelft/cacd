...
void raw_write(name, pl, app, binary)
    char *name;
    struct plot *pl;
    bool app, binary;
{
    ...
    for (vv = pl->pl_env; vv; vv = vv->va_next) {
        if (vv->va_type == VT_BOOL) {
            ...
            if (vv->va_type == VT_LIST) fprintf(fp, "( ");
	    wl = cp_varwl(vv);
            wl_print(wl, fp);
            wl_free(wl);
            if (vv->va_type == VT_LIST) fprintf(fp, " )");
	...
	if (v->v_numdims != numdims) {
	  writedims = true;
	    ...
	    for (j = 0; j < numdims; j++)
		if (dims[j] != v->v_dims[j]) writedims = true;
	}
	if (writedims) {
...
struct plot * raw_read (char *name)
{
    ...
    while (fgets(buf, BSIZE_SP, fp)) {
        ...
        if (ciprefix("title:", buf)) {
            ...
            if (curpl) {    /* reverse commands list */
                for (wl=curpl->pl_commands,
                        curpl->pl_commands=NULL; wl && wl->wl_next; wl=nwl) {
                    nwl = wl->wl_next;
	    ...
	    if (npoints == 0) {
		fprintf(cp_err, "Error: misplaced Dimensions: line\n");
		continue;
	    }
            s = buf;
            skip(s);
	    if (atodims(s, dims, &numdims)) { /* Something's wrong. */
		fprintf(cp_err, "Warning: syntax error in dimensions, ignored.\n");
		numdims = 0;
		continue;
	    }
	    if (numdims > MAXDIMS) {
	    ...
	    if (ndimpoints != npoints) {
                fprintf(cp_err, "Warning: dimensions inconsistent with no. of points, ignored.\n");
		numdims = 0;
	    }
            ...
            if (curpl) {
                wl = cp_lexer(s);
                for (vv = curpl->pl_env; vv && vv->va_next; vv = vv->va_next) ;
                if (vv)
		...
		ZERO(v, struct dvec);
                ...
                curpl->pl_dvecs = v;
                if (!curpl->pl_scale) curpl->pl_scale = v;
                v->v_flags = flags;
                v->v_plot = curpl;
                v->v_length = npoints;
		v->v_numdims = 0;
		/* Length and dims might be changed by options. */
                ...
                /* Fix the name... */
                if (isdigit(*v->v_name) && (r = ft_typabbrev(v->v_type))) {
                    sprintf(buf2, "%s(%s)", r, v->v_name);
                    v->v_name = copy(buf2);
                }
		 ...
		 */
                if (isreal(v))
                    v->v_realdata = (double *) tmalloc(npoints * sizeof(double));
                else
                    v->v_compdata = (complex *) tmalloc(npoints * sizeof(complex));
            }
			...
			if (i < v->v_length) {
				...
				if (fscanf(fp, " %lf", &v->v_realdata[i]) != 1)
				    fprintf(cp_err, "Error: bad rawfile\n");
			    } else {
				if (fscanf(fp, " %lf, %lf",
					&realpart(&v->v_compdata[i]),
			...
			if (i < v->v_length) {
			    if (flags & VF_REAL) {
				if (fread((char *) &v->v_realdata[i], sizeof(double), 1, fp) != 1)
				    fprintf(cp_err, "Error: bad rawfile\n");
			    } else {
				if (fread((char *) &v->v_compdata[i].cx_real, sizeof(double), 1, fp) != 1)
				    fprintf(cp_err, "Error: bad rawfile\n");
				if (fread((char *) &v->v_compdata[i].cx_imag, sizeof(double), 1, fp) != 1)
				    fprintf(cp_err, "Error: bad rawfile\n");
			    }
			} else if (raw_padded) {
			    if (flags & VF_REAL) {
				if (fread((char *) &junk, sizeof(double), 1, fp) != 1)
				    fprintf(cp_err, "Error: bad rawfile\n");
			    } else {
				if (fread((char *) &junk, sizeof(double), 1, fp) != 1)
				    fprintf(cp_err, "Error: bad rawfile\n");
            ...
            nonl(s);
            if (*s) {
                fprintf(cp_err, "Error: strange line in rawfile;\n\t\"%s\"\nload aborted.\n", s);
                return (NULL);
            }
        }
    }
    ...
    if (curpl) {    /* reverse commands list */
        for (wl=curpl->pl_commands, curpl->pl_commands=NULL; wl && wl->wl_next; wl=nwl) {
            nwl = wl->wl_next;
            wl->wl_next = curpl->pl_commands;
            curpl->pl_commands = wl;
        }
