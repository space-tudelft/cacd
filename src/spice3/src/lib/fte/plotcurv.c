...
void ft_graf(struct dvec *v, struct dvec *xs, bool nostart)
{
    ...
    if (nostart) {
      degree = currentgraph->degree;
        ...
        if (!cp_getvar("polydegree", VT_NUM, (char *) &degree)) degree = 1;
        currentgraph->degree = degree;
    }
    if (degree > v->v_length) degree = v->v_length;
    if (degree < 1) {
        fprintf(cp_err, "Error: polydegree is %d, can't plot...\n", degree);
        return;
    }

    if (!cp_getvar("gridsize", VT_NUM, (char *) &gridsize)) gridsize = 0;
      ...
      if (isreal(xs)) {
        ...
        for (i = 0; i < xs->v_length - 1; i++)
            if (increasing != (xs->v_realdata[i] < xs->v_realdata[i + 1])) {
                fprintf(cp_err, "Warning: scale not monotonic, gridsize not relevant.\n");
                gridsize = 0;
                break;
            }
       } else {
	...
	increasing = 0; /* prevent compiler warning */

    if (!nostart) gr_start(v);
 ...
 */
	...
	dir = 0;
	dx = dy = 0; /* init, prevent compiler warning */
        for (i = 0, j = v->v_length; i < j; i++) {
            dx = isreal(xs) ? xs->v_realdata[i] : realpart(&xs->v_compdata[i]);
            dy = isreal(v) ? v->v_realdata[i] : realpart(&v->v_compdata[i]);
    ...
    if (gridsize) {
        ...
        gridbuf = allocn(double, gridsize);
        result  = allocn(double, gridsize);
        if (isreal(v))
            ydata = v->v_realdata;
        else {
            ydata = allocn(double, v->v_length);
            for (i = 0; i < v->v_length; i++)
        ...
        if (isreal(xs))
            ...
            xdata = allocn(double, xs->v_length);
        ...
        if (increasing)
            ...
            for (i = 0, dy = mm[1]; i < gridsize; i++, dy -= dx)
                gridbuf[i] = dy;
        if (!ft_interpolate(ydata, result, xdata, v->v_length, gridbuf, gridsize, degree)) {
        ...
        tfree(gridbuf);
        tfree(result);
        if (!isreal(v)) tfree(ydata);
        if (!isreal(xs)) tfree(xdata);
        return;
    }
     ...
     * space
     */
    scratch = allocn(double, (degree + 1) * (degree + 2));
    result  = allocn(double, (degree + 1));
    xdata   = allocn(double, (degree + 1));
    ydata   = allocn(double, (degree + 1));

    /* Plot the first degree segments... */
    if (isreal(v))
        bcopy((char *) v->v_realdata, (char *) ydata, (degree + 1) * sizeof (double));
    else
        for (i = 0; i <= degree; i++)
            ydata[i] = realpart(&v->v_compdata[i]);
    if (isreal(xs))
        bcopy((char *) xs->v_realdata, (char *) xdata, (degree + 1) * sizeof (double));
    else
        for (i = 0; i <= degree; i++)
            xdata[i] = realpart(&xs->v_compdata[i]);
    ...
    for (l = degree + 1; l < length; l++) {
        ...
        if (rot)
            plotinterval(v, ydata[degree - 1], ydata[degree], result, degree, true);
        else
            plotinterval(v, xdata[degree - 1], xdata[degree], result, degree, false);
    }
    tfree(scratch);
    tfree(xdata);
    tfree(ydata);
...
static void plotinterval(struct dvec *v, double lo, double hi, double *coeffs, int degree, bool rotated)
{
    ...
    for (i = 0; i <= degree; i++) fprintf(cp_err, "%G ", coeffs[i]);
...
static void printmat(char *name, double *mat, int m, int n)
{
    ...
    for (i = 0; i < m; i++) {
        printf(" | ");
        for (j = 0; j < n; j++) printf("%G ", mat[i * n + j]);
        printf("|\n\r");
    }
    printf("===\n\r");
    return;
