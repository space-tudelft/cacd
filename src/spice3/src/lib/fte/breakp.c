...
void dbfree (struct dbcomm *db)
{
    ...
    for (dd = db; dd; dd = dn) {
        dn = dd->db_also;
        txfree(dd->db_nodename1);
        txfree(dd->db_nodename2);
        txfree(dd);
    }
}
...
static bool satisfied (struct dbcomm *d, struct plot *plot)
{
    ...
    switch (d->db_op) {
        ...
        case DBC_LT:
            return ((d1 < d2) ? true : false);
        default:
            fprintf(cp_err, "satisfied: Internal Error: bad cond %d\n", d->db_op);
            return (false);
    }
}
    ...
    for (dt = d; dt; dt = dt->db_also) {
        if (dt->db_type == DB_STOPAFTER)
            ...
            if (dt->db_nodename1)
                ...
                fprintf(fp, " when %lg", dt->db_value1);
            switch (dt->db_op) {
                case DBC_EQU:
                    fputs(" =", fp);
                    break;
                ...
                case DBC_LTE:
                    ...
                    break;
                default:
                    fprintf(cp_err, "printcond: Internal Error: bad cond %d", dt->db_op);
            }
            if (dt->db_nodename2)
                fprintf(fp, " %s", dt->db_nodename2);
            else
                fprintf(fp, " %lg", dt->db_value2);
        }
    }
}
