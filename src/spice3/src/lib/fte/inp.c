...
void com_listing (wordlist *wl)
{
    ...
    if (ft_curckt) {
        ...
        inp_list(cp_out, expand ? ft_curckt->ci_deck :
                ft_curckt->ci_origdeck, ft_curckt->ci_options, type);
    } else
        fprintf(cp_err, "Error: no circuit loaded.\n");
}

static char * inp_casefix2 (char *string)
{
	...
	for (here = deck; here; here = here->li_next) {
			...
			out_send(out_pbuf);
                ...
                for (there = here->li_actual; there; there = there->li_next) {
                    ...
                    if (type == LS_PHYSICAL) {
                        if (useout) {
                            sprintf(out_pbuf, "%6d : %s\n", there->li_linenum, inp_casefix2(there->li_line));
			    out_send(out_pbuf);
                        } else
                            fprintf(file, "%6d : %s\n", there->li_linenum, inp_casefix2(there->li_line));
                    } else {
                        if (useout)
                            out_printf("%s\n", inp_casefix2(there->li_line));
                        else
                            fprintf(file, "%s\n", inp_casefix2(there->li_line));
                    }
...
void inp_spsource (FILE *fp, bool comfile, char *filename)
{
    ...
    if (!comfile)
        ...
        options = NULL; /* prevent compiler warning */
    ...
    if (!comfile) {
        ...
        if (!deck->li_next) fprintf(cp_err, "Warning: no lines in input\n");
    } else
        tt = NULL; /* prevent compiler warning */
    ...
    if (comfile) {
        ...
        if (controls) {
            wl = controls; wl->wl_prev = NULL;
            while ((end = wl->wl_next)) { end->wl_prev = wl; wl = end; }
	    do {
                if (prefix("*#", wl->wl_word))
		    (void) cp_evloop(wl->wl_word + 2);
                else
...
void inp_dodeck(deck, tt, end, reuse, options, filename)
    struct line *deck;
    char *tt;
    wordlist *end;
    bool reuse;
    ...
    char *filename;
{
    ...
    if (reuse) {
        ...
        if (ft_curckt) {
            ft_curckt->ci_devices = cp_kwswitch(CT_DEVNAMES, (char *) NULL);
...
#ifdef notdef
	/* Unused time-waster. */
        for (dd = deck->li_next; dd; dd = dd->li_next) if_setndnames(dd->li_line);
#endif
    ...
    if (!noparse) {
	    ...
	    case VT_BOOL:
                ...
                break;
	    case VT_NUM:
                if_option(ct->ci_ckt, eev->va_name, eev->va_type, (char *) &eev->va_num);
                break;
	    case VT_REAL:
                if_option(ct->ci_ckt, eev->va_name, eev->va_type, (char *) &eev->va_real);
                break;
	    case VT_STRING:
 ...
 * used on a non-unix machine, they will leave spice.tmp junk files lying around.
 */
void com_edit (wordlist *wl)
{
    ...
    if (wl) {
        ...
        if (ft_curckt && !ft_curckt->ci_filename) {
            ...
            fprintf(cp_err, "Warning: editing a temporary file -- circuit not saved\n");
            fclose(fp);
        } else if (!ft_curckt) {
            if (!(fp = fopen(filename, "w"))) {
                perror(filename);
        ...
        if (ft_curckt && !ft_curckt->ci_filename) unlink(filename);
    }
