 ...
 *
 * NOTE: most of this doesn't work -- there will be plenty of unused variable messages
 */
...
void com_compose(wordlist *wl)
{
    ...
    resname = cp_unquote(wl->wl_word); /* copy() */
    vec_remove(resname);
    ...
    if (eq(wl->wl_word, "values")) {
        ...
        wl = wl->wl_next;
        if (!(pn = ft_getpnames(wl, true))) return;
	first_pn = pn;
        while (pn) {
            if (!(v = ft_evaluate(pn))) return;
            if (!vecs)
                vecs = lv = v;
            else
                lv->v_link2 = v;
        ...
        for (v = vecs->v_link2, length = 1; v; v = v->v_link2) {
            ...
            if (i < 2)
                i = (v->v_length > 1) ? 1 : 0;
            if (i != dim) {
                fprintf(cp_err, "Error: all vectors must be of the same dimensionality\n");
                return;
            }
            length++;
            if (iscomplex(v))
        ...
        for (i = 0, blocksize = 1; i < dim - 1; i++)
            blocksize *= dims[i];
        if (realflag)
            data = allocn(double, length * blocksize);
        else
            cdata = allocn(complex, length * blocksize);
        ...
        while (wl) {
	    var = wl->wl_word;
	    if ((val = strchr(var, '='))) { /* var=val */
		*val++ = '\0';
		if (!*val) { /* This is var= val. */
		    wl = wl->wl_next;
		    if (wl) val = wl->wl_word;
		}
	    } else { /* var =val */
		wl = wl->wl_next;
		if (wl) {
	    ...
	    td = NULL;
            if (cieq(var, "start")) {
                startgiven = true;
                if ((td = ft_numparse(&val, false))) start = *td;
            } else if (cieq(var, "stop")) {
                stopgiven = true;
                if ((td = ft_numparse(&val, false))) stop = *td;
            } else if (cieq(var, "step")) {
                stepgiven = true;
                if ((td = ft_numparse(&val, false))) step = *td;
            } else if (cieq(var, "center")) {
                centergiven = true;
                if ((td = ft_numparse(&val, false))) center = *td;
            } else if (cieq(var, "span")) {
                spangiven = true;
                if ((td = ft_numparse(&val, false))) span = *td;
            } else if (cieq(var, "mean")) {
                meangiven = true;
                if ((td = ft_numparse(&val, false))) mean = *td;
            } else if (cieq(var, "sd")) {
                sdgiven = true;
                if ((td = ft_numparse(&val, false))) sd = *td;
            } else if (cieq(var, "lin")) {
                lingiven = true;
                if ((td = ft_numparse(&val, false))) lin = *td;
            } else if (cieq(var, "log")) {
                loggiven = true;
                if ((td = ft_numparse(&val, false))) log = *td;
            } else if (cieq(var, "dec")) {
                decgiven = true;
                if ((td = ft_numparse(&val, false))) dec = *td;
            } else if (cieq(var, "gauss")) {
                gaussgiven = true;
                if ((td = ft_numparse(&val, false))) gauss = *td;
            } else if (cieq(var, "random")) {
                randmgiven = true;
                if ((td = ft_numparse(&val, false))) randm = *td;
	    ...
	    if (!td) {
		fprintf(cp_err, "Error: bad parm %s = %s\n", var, val);
		return;
	    }
        }
        ...
        if (lingiven + loggiven + decgiven + randmgiven + gaussgiven > 1) {
            fprintf(cp_err, "Error: can have at most one of (lin, log, dec, random, gauss)\n");
            return;
            ...
            if (startgiven && stopgiven && stepgiven) {
                ...
                fprintf(cp_err, "Error: either one of (lin, log, dec, random, gauss) must be given, or all\n");
        ...
        if (lingiven) {
            ...
            data = allocn(double, (int)lin);
            if (stepgiven && startgiven && stopgiven) {
                if (step != (stop - start) / lin * (reverse ? -1 : 1)) {
                    fprintf(cp_err, "Warning: bad step -- should be %lg\n",
...
static void dimxpand(v, newdims, data)
    struct dvec *v;
    int *newdims;
    double *data;
{
