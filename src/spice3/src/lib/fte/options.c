...
struct variable * cp_enqvar (char *word)
{
    ...
    if (*word == '&') {
        ...
        if (d) {
            ...
            if (d->v_link2)
                fprintf(cp_err, "Warning: only one vector may be accessed with the $& notation.\n");
            return (vv);
        }
	return (NULL);
    }
    ...
    if (plot_cur) {
        for (vv = plot_cur->pl_env; vv; vv = vv->va_next)
            if (eq(vv->va_name, word)) return (vv);
...
int cp_usrset (struct variable *var, bool isset)
{
    ...
    */
    ...
    switch (var->va_type) {
	    ...
	    break;
        case VT_STRING:
            strcpy(val, var->va_string);
            vv = val;
            break;
        case VT_NUM:
            /*sprintf(val, "%d", var->va_num);*/
            iv = var->va_num;
            vv = (char *) &iv;
            break;
        case VT_REAL:
            /*strcpy(val, printnum(var->va_real));*/
            dv = var->va_real;
            vv = (char *) &dv;
            break;
        case VT_LIST:
	    ...
	    vv = NULL;
            break;
        default:
            fprintf(cp_err, "cp_usrset: Internal Error: bad var type %d\n", var->va_type);
	    return (0);
    }
