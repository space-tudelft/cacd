...
/*
 * Expand subcircuits. This is very spice-dependent.
 * Bug fixes by Norbert Jeske on 10/5/85.
 */
...
struct line * inp_subcktexpand (struct line *deck)
{
    ...
    if (!cp_getvar("substart",  VT_STRING, start)) strcpy(start, ".subckt");
    if (!cp_getvar("subend",    VT_STRING, sbend)) strcpy(sbend, ".ends");
    if (!cp_getvar("subinvoke", VT_STRING, invoke)) strcpy(invoke, "X");
    if (!cp_getvar("modelcard", VT_STRING, model)) strcpy(model, ".model");
    if (!cp_getvar("modelline", VT_STRING, model)) strcpy(model, ".model");
...
static struct line * doit (struct line *deck)
{
    ...
    for (c = deck; c; c = c->li_next)
        if (prefix(model, c->li_line)) {
            ...
            wl = alloc(wordlist);
            wl->wl_next = modnames;
            if (modnames) modnames->wl_prev = wl;
            modnames = wl;
            wl->wl_word = gettok(&s);
        }
    ...
    if (error) return NULL; /* error message already reported; should free() */
 ...
 * touch it.
 */
static int translate(struct line *deck, char *formal, char *actual, char *scname, char *subname)
{
    struct line *c;
    ...
    if (i < 0) {
	fprintf(stderr, "Too few parameters for subcircuit type \"%s\" (instance: x%s)\n", subname, scname);
	return 0;
    } else if (i > 0) {
	fprintf(stderr, "Too many parameters for subcircuit type \"%s\" (instance: x%s)\n", subname, scname);
	return 0;
    }
    ...
    for (c = deck; c; c = c->li_next) {
		...
		continue;
            ...
            if (*name == ':') name++;
            if (*name)
                sprintf(buffer + strlen(buffer), "%c:%s:%s ", ch, scname, name);
            else
                sprintf(buffer + strlen(buffer), "%c:%s ", ch, scname);
            }
...
static void finishLine(char *dst, char *src, char *scname)
{
	...
	lastwasalpha = 0;
        ...
        *dst++ = *src++;
        ...
        for (i = 0; *src && !isspace(*src) && *src != ',' && *src != ')'; i++)
	{
            buf[i] = *src++;
	}
        buf[i] = '\0';
...
static char * gettrans(char *name)
{
    ...
    if (eq(name, "0")) return (name);
    for (i = 0; table[i].t_old; i++)
        if (eq(table[i].t_old, name)) return (table[i].t_new);
    return (NULL);
}
...
static bool modtranslate(struct line *deck, char *subname)
{
    ...
    for (c = deck; c; c = c->li_next) {
        if (prefix(model, c->li_line)) {
            ...
            t = c->li_line;
            name = gettok(&t);
            buffer = tmalloc(strlen(name) + strlen(t) + strlen(subname) + 4);
            sprintf(buffer, "%s ", name);
            name = gettok(&t);
            wlsub = alloc(wordlist);
            wlsub->wl_next = submod;
            if (submod) submod->wl_prev = wlsub;
            submod = wlsub;
            wlsub->wl_word = name;
            sprintf(buffer + strlen(buffer), "%s:%s ", subname, name);
            strcat(buffer, t);
            tfree(c->li_line);
            c->li_line = buffer;
            t = c->li_line;
