...
/* Returns the minimum and maximum values of a dvec.
 * Returns a pointer to static data.
 * If real is true look at the real parts, otherwise the imag parts.
 */
...
int ft_findpoint(double pt, double *lims, int maxp, int minp, bool islog)
{
    ...
    if (pt < lims[0]) pt = lims[0];
    if (pt > lims[1]) pt = lims[1];
    if (islog) {
        tl = mylog10(lims[0]);
        th = mylog10(lims[1]);
        return (((mylog10(pt) - tl) / (th - tl)) * (maxp - minp) + minp);
    }
    return (((pt - lims[0]) / (lims[1] - lims[0])) * (maxp - minp) + minp);
}
...
double * ft_SMITHminmax(struct dvec *v, bool yval)
{
    ...
    for (i = 0; i < v->v_length; i++) {
	    ...
	    SMITH_tfm(v->v_realdata[i], 0.0, &d, &d2);
            ...
            SMITH_tfm(realpart(&v->v_compdata[i]), imagpart(&v->v_compdata[i]), &d, &d2);
/* Are we are looking for min/max X or Y value
 */
	if (yval) d = d2;
        if (d < res[0]) res[0] = d;
        if (d > res[1]) res[1] = d;
...
void SMITH_tfm(double re, double im, double *x, double *y)
{
