...
static void drawlingrid(), drawloggrid();
    ...
    Axis axis;
{
	...
	*buf = 0;
	...
	sprintf(buf, "x10^%d ", mag);
    } else
        tenpowmag = 1; /* prevent compiler warning */
    ...
    Axis axis;
{
	...
	sprintf(buf, "%.*f", digits + 1, m * mag / 100.0);

#ifdef notdef
	if (floor(step/10.0) != step/10.0)
		sprintf(buf, "%.*lf", mag, m * mag / 100.0);
	else if (floor(step/100.0) != step/100.0)
		sprintf(buf, "%.1lf", m * mag / 100.0);
	else
		sprintf(buf, "%lg", j * mag / 100);
#endif
    ...
    Axis axis;
{
    ...
    if (axis == x_axis) {
      strcpy(graph->grid.xaxis.log.units, buf);
      ...
      strcpy(graph->grid.yaxis.log.units, buf);
...
GRAPH *graph;
{
     ...
     */
    j = 10; /* use some default, prevent compiler warning */
