...
void com_linearize (wordlist *wl)
{
    ...
    if (!ft_curckt || !ft_curckt->ci_ckt ||
        ...
        fprintf(cp_err, "Error: can't get transient parameters from circuit\n");
        return;
    }
    if (((tstop - tstart) * tstep <= 0.0) || ((tstop - tstart) < tstep)) {
        fprintf(cp_err, "Error: bad parameters -- start = %G, stop = %G, step = %G\n",
                tstart, tstop, tstep);
        return;
    }
    ...
    sprintf(buf, "%s (linearized)", old->pl_name);
    ...
    newtime->v_plot = new;
    newtime->v_realdata = allocn(double, len);
