...
static struct dvec *doop(int what, char *(*func)(), struct pnode *arg1, struct pnode *arg2);
static char *mkcname(int what, char *v1, char *v2);

/* We are careful here to catch SIGILL and recognise them as math errors.
 * The only trouble is that the signal handler we installed before will
...
struct dvec * ft_evaluate(struct pnode *node)
{
    ...
    if (node->pn_value)
        ...
        if (node->pn_op->op_arity == 1)
            d = (struct dvec *) ((*node->pn_op->op_func) (node->pn_left));
        else if (node->pn_op->op_arity == 2)
            d = (struct dvec *) ((*node->pn_op->op_func) (node->pn_left, node->pn_right));
    } else {
...
struct dvec * op_range(struct pnode *arg1, struct pnode *arg2)
{
    ...
    for (i = len = 0; i < scale->v_length; i++) {
        td = isreal(scale) ? scale->v_realdata[i] : realpart(&scale->v_compdata[i]);
        if ((td <= up) && (td >= low)) len++;
    }
    ...
    */
    ...
    if (isreal(res))
        res->v_realdata = allocn(double, len);
    else
        res->v_compdata = allocn(complex, len);
    ...
    for (i = (rev ? v->v_length - 1 : 0); i != (rev ? -1 : v->v_length);
        ...
        if ((td <= up) && (td >= low)) {
            if (isreal(res)) {
                ...
                realpart(&res->v_compdata[j]) = realpart(&v->v_compdata[i]);
                imagpart(&res->v_compdata[j]) = imagpart(&v->v_compdata[i]);
            }
            j++;
        }
    }
...
struct dvec * op_ind(struct pnode *arg1, struct pnode *arg2)
{
    ...
    if (isreal(res))
        res->v_realdata = allocn(double, length);
    else
        res->v_compdata = allocn(complex, length);
...
static struct dvec * apply_func(struct func *func, struct pnode *arg)
{
    ...
    if (!func->fu_func) {
        ...
        sprintf(buf, "v(%s)", arg->pn_value->v_name);
    ...
    for (; v; v = v->v_link2) {
        ...
        if (eq(func->fu_name, "interpolate") || eq(func->fu_name, "deriv"))       /* Ack */
	{
            data = (char *) ((*func->fu_func) ((isreal(v) ? (char *)v->v_realdata : (char *)v->v_compdata),
                (isreal(v) ? VF_REAL : VF_COMPLEX), v->v_length, &len, &type, v->v_plot, plot_cur, v->v_dims[0]));
        } else {
            data = (char *) ((*func->fu_func) ((isreal(v) ? (char *)v->v_realdata : (char *)v->v_compdata),
                (isreal(v) ? VF_REAL : VF_COMPLEX), v->v_length, &len, &type));
	}
        /* Back to normal */
        (void) signal(SIGILL, SIG_DFL);
	...
	ZERO(t, struct dvec);
        ...
        t->v_flags = (v->v_flags & ~VF_COMPLEX & ~VF_REAL & ~VF_PERMANENT & ~VF_MINGIVEN & ~VF_MAXGIVEN);
        t->v_flags |= type;
#ifdef FTEDEBUG
        if (ft_evdb)
            fprintf(cp_err, "apply_func: func %s to %s len %d, type %d\n",
...
static struct dvec * doop(int what, char *(*func)(), struct pnode *arg1, struct pnode *arg2)
{
	...
	if (v1->v_numdims != v2->v_numdims) {
	    fprintf(cp_err, "Warning: operands %s and %s have incompatible shapes.\n",
		v1->v_name, v2->v_name);
	    return (NULL);
	}
    ...
    /* Don't bother to do type checking.  Maybe this should go in at some point. */
	...
	if (isreal(v1)) {
	    ld = 0.0;
	    c1 = NULL;
	    d1 = allocn(double, length);
	    for (i = 0; i < v1->v_length; i++) d1[i] = v1->v_realdata[i];
	    if (length > 0)
		ld = v1->v_realdata[v1->v_length - 1];
	    for ( ; i < length; i++) d1[i] = ld;
	} else {
	    realpart(&lc) = 0.0;
	    imagpart(&lc) = 0.0;
	    c1 = allocn(complex, length);
	    d1 = NULL;
	    for (i = 0; i < v1->v_length; i++) c1[i] = v1->v_compdata[i];
	    if (length > 0)
		lc = v1->v_compdata[v1->v_length - 1];
	    for ( ; i < length; i++) c1[i] = lc;
	}
    } else {
	if (isreal(v1)) {
	    c1 = NULL;
	...
	if (isreal(v2)) {
	    ld = 0.0;
	    c2 = NULL;
	    d2 = allocn(double, length);
	    for (i = 0; i < v2->v_length; i++) d2[i] = v2->v_realdata[i];
	    if (length > 0)
		ld = v2->v_realdata[v2->v_length - 1];
	    for ( ; i < length; i++) d2[i] = ld;
	} else {
	    realpart(&lc) = 0.0;
	    imagpart(&lc) = 0.0;
	    c2 = allocn(complex, length);
	    d2 = NULL;
	    for (i = 0; i < v2->v_length; i++) c2[i] = v2->v_compdata[i];
	    if (length > 0)
		lc = v2->v_compdata[v1->v_length - 1];
	    for ( ; i < length; i++) c2[i] = lc;
	}
    } else {
	if (isreal(v2)) {
	    c2 = NULL;
     ...
     * out of range.  Catch this here.
     */
    if (setjmp(matherrbuf)) return (NULL);
    ...
    data = (char *) ((*func) ((isreal(v1) ? (char *) d1 : (char *) c1),
                  ...
                  (isreal(v2) ? VF_REAL : VF_COMPLEX), length));
    /* Back to normal */
    (void) signal(SIGILL, SIG_DFL);
    ...
    if (relflag || (isreal(v1) && isreal(v2) && (func != cx_comma))) {
        res->v_flags = (v1->v_flags | v2->v_flags | VF_REAL) & ~VF_COMPLEX;
        res->v_realdata = (double *) data;
    } else {
        res->v_flags = (v1->v_flags | v2->v_flags | VF_COMPLEX) & ~VF_REAL;
	...
	for (i = 0; i < v1->v_numdims; i++) res->v_dims[i] = v1->v_dims[i];
    } else {
	res->v_numdims = v2->v_numdims;
	for (i = 0; i < v2->v_numdims; i++) res->v_dims[i] = v2->v_dims[i];
    }
    ...
    if (free1) { if (isreal(v1)) { txfree(d1); } else { txfree(c1); } }
    if (free2) { if (isreal(v2)) { txfree(d2); } else { txfree(c2); } }
