...
void inp_nutsource (FILE *fp, bool comfile, char *filename)
{
    ...
    if (!comfile) {
        ...
        if (!deck->li_next) fprintf(cp_err, "Warning: no lines in deck...\n");
    } else
        tt = NULL; /* init, prevent compiler warning */
    ...
    if (comfile) {
        ...
        for (dd = deck->li_next; dd; dd = ld->li_next) {
            ...
            *t = '\0';
            ...
            if (ciprefix(".control", dd->li_line)) {
                ...
                wl = alloc(wordlist);
		wl->wl_next = controls;
                if (controls) controls->wl_prev = wl;
		controls = wl;
        ...
        if (deck->li_next) {
            ...
            /* Now expand subcircuit macros. Note that we have to fix the case
	     * before we do this but after we deal with the commands.
             */
...
void nutinp_dodeck(deck, tt, end, reuse, options, filename)
    struct line *deck;
    char *tt;
    wordlist *end;
    bool reuse;
