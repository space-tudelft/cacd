...
struct pnode * ft_getpnames(wordlist *wl, bool check)
{
    ...
    txfree(xsbuf);
    if (check && !checkvalid(pn)) return (NULL);
    return (pn);
}
...
static bool checkvalid(struct pnode *pn)
{
    while (pn) {
        if (pn->pn_value) {
            if ((pn->pn_value->v_length == 0) && !eq(pn->pn_value->v_name, "list")) {
                if (eq(pn->pn_value->v_name, "all"))
                    fprintf(cp_err, "Error: %s: no matching vectors.\n", pn->pn_value->v_name);
                else
                    fprintf(cp_err, "Error: %s: no such vector.\n", pn->pn_value->v_name);
                return (false);
            }
        } else if (pn->pn_func || (pn->pn_op && (pn->pn_op->op_arity == 1))) {
            if (!checkvalid(pn->pn_left)) return (false);
        } else if (pn->pn_op && (pn->pn_op->op_arity == 2)) {
            if (!checkvalid(pn->pn_left)) return (false);
            if (!checkvalid(pn->pn_right)) return (false);
        } else
            fprintf(cp_err, "checkvalid: Internal Error: bad node\n");
        pn = pn->pn_next;
    }
    return (true);
}
...
static struct element * lexer()
{
    ...
    case '(':
        if (((lasttoken == VALUE) && ((lasttype == NUM))) || (lasttoken == RPAREN)) {
            el = end;
            goto done;
        } else {
            el.e_token = LPAREN;
    ...
    case '>':
    case '<':
        for (j = 0; isspace(sbuf[j]); j++) ; /* The lexer makes <> into < > */
    ...
    if ((!ss || *ss != ':') && td) {
        ...
        if ((sbuf[0] == 'g') && (sbuf[1] == 't') && strchr(specials, sbuf[2])) {
            el.e_token = GT;
            sbuf += 2;
        } else if ((sbuf[0] == 'l') && (sbuf[1] == 't') && strchr(specials, sbuf[2])) {
            el.e_token = LT;
            sbuf += 2;
        } else if ((sbuf[0] == 'g') && (sbuf[1] == 'e') && strchr(specials, sbuf[2])) {
            el.e_token = GE;
            sbuf += 2;
        } else if ((sbuf[0] == 'l') && (sbuf[1] == 'e') && strchr(specials, sbuf[2])) {
            el.e_token = LE;
            sbuf += 2;
        } else if ((sbuf[0] == 'n') && (sbuf[1] == 'e') && strchr(specials, sbuf[2])) {
            el.e_token = NE;
            sbuf += 2;
        } else if ((sbuf[0] == 'e') && (sbuf[1] == 'q') && strchr(specials, sbuf[2])) {
            el.e_token = EQ;
            sbuf += 2;
        } else if ((sbuf[0] == 'o') && (sbuf[1] == 'r') && strchr(specials, sbuf[2])) {
            el.e_token = OR;
            sbuf += 2;
        } else if ((sbuf[0] == 'a') && (sbuf[1] == 'n') && (sbuf[2] == 'd') && strchr(specials, sbuf[3])) {
            el.e_token = AND;
            sbuf += 3;
        } else if ((sbuf[0] == 'n') && (sbuf[1] == 'o') && (sbuf[2] == 't') && strchr(specials, sbuf[3])) {
            el.e_token = NOT;
            sbuf += 3;
        } else {
            if ((lasttoken == VALUE) || (lasttoken == RPAREN)) {
             ...
             */
	    atsign = 0;
            for (s = el.e_string; *s && !strchr(specials, *s); s++, sbuf++) {
                if (*s == '@')
		    atsign = 1;
                else if (*s == '[' && !atsign)
                    break;
...
static struct pnode * parse()
{
	...
	case E:
            ...
            bcopy((char *) next, (char *) &stack[++sp], sizeof(struct element));
            next = lexer();
            continue;
	...
	case G:
            ...
            while (sp > 0) {
                if (stack[sp - 1].e_token == VALUE)
                    ...
                    i = 1;
                if (prectable[stack[sp - i].e_token][stack[sp].e_token] == L)
                    break;
                else
                    sp = sp - i;
            }
            ...
            if (st == sp) {
                ...
                if (pn == NULL) goto err;
                    ...
                    (stack[sp].e_token == NOT)) && (st == sp + 1)) {
                lpn = makepnode(&stack[st]);
                if (lpn == NULL) goto err;
                ...
                if ((lpn == NULL) || (stack[sp].e_type != STRING)) goto err;
                if (!(pn = mkfnode(stack[sp].e_string, lpn))) return (NULL);
            } else { /* node op node */
                lpn = makepnode(&stack[sp]);
                rpn = makepnode(&stack[st]);
                if ((lpn == NULL) || (rpn == NULL)) goto err;
                pn = mkbnode(stack[sp + 1].e_token, lpn, rpn);
            }
...
static struct pnode * mkfnode(char *func, struct pnode *arg)
{
    ...
    if ((f->fu_name == NULL) && arg->pn_value) {
        ...
        sprintf(buf, "%s(%s)", func, arg->pn_value->v_name);
        d = vec_get(buf);
        if (d == NULL) {
            /* Well, too bad. */
	...
	p = mkbnode(MINUS, mkfnode(func, arg->pn_left), mkfnode(func, arg->pn_right));
	txfree(arg);
	return p;
    }
