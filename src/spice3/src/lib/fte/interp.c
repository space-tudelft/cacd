...
/* Interpolate data from oscale to nscale.
 * The data is assumed to be olen long, ndata will be nlen long.
 * Returns false if the scales are too strange to deal with.
 * Note that we are guaranteed that either both scales are
...
bool ft_interpolate(double *data, double *ndata, double *oscale, int olen, double *nscale, int nlen, int degree)
{
    ...
    lastone = -1;
    for (i = 0; i < degree; i++) {
        lastone = putinterval(result, degree, ndata, lastone, nscale, nlen, xdata[i], sign);
    }
    ...
    for (l = degree + 1; l < olen; l++) {
        ...
        while (!ft_polyfit(xdata, ydata, result, degree, scratch)) {
            if (--degree == 0) {
                fprintf(cp_err, "interpolate: Internal Error.\n");
                return (false);
            }
        }
        lastone = putinterval(result, degree, ndata, lastone, nscale, nlen, xdata[i], sign);
...
#define REL_TOL 0.001
    ...
    for (i = 0; i < n; i++) {
        ...
        if (fabs(d - ydata[i]) > ABS_TOL) {
            /*
            fprintf(cp_err, "Error: polyfit: x = %le, y = %le, int = %le\n", xdata[i], ydata[i], d);
            printmat("mat1", mat1, n, n);
            printmat("mat2", mat2, n, 1);
            */
            return (false);
        } else if (fabs(d - ydata[i]) / (fabs(d) > ABS_TOL ? fabs(d) : ABS_TOL) > REL_TOL) {
            /*
            fprintf(cp_err, "Error: polyfit: x = %le, y = %le, int = %le\n", xdata[i], ydata[i], d);
            printmat("mat1", mat1, n, n);
            printmat("mat2", mat2, n, 1);
 ...
 * from, and sign is 1 if the old scale was increasing, and -1 if it was decreasing.
 */
static int putinterval(double *poly, int degree, double *nvec, int last, double *nscale, int nlen, double oval, int sign)
{
    int end, i;

    /* See how far we have to go. */
    for (end = last + 1; end < nlen; end++)
        if (nscale[end] * sign > oval * sign) break;
    end--;
...
double ft_peval(double pt, double *coeffs, int degree)
{
    register int i, j;
    double d = 0.0, f;
...
{
    int i;
    for (i = 0; i < degree; i++) coeffs[i] = (i + 1) * coeffs[i + 1];
}
