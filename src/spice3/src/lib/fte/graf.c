...
static void drawlegend (GRAPH *, int, struct dvec *);
static void gr_resize_internal (GRAPH *);
static void gr_restoretext (GRAPH *);
static double *readtics (char *string);
 ...
 *    leave that to lower level routines.
 */
/* ARGSUSED */ /* since we don't really use many of them */
bool gr_init(double *xlims, double *ylims, /* The size of the screen. */
	...
	char *commandline)	/* For xi_zoomdata() */
{
    ...
    if (!(graph = NewGraph())) return(false);
    ...
    /* set slow flag to stop between each plot and prompt the user for a return
...
static void drawlegend (GRAPH *graph, int plotno, struct dvec *dv)
{
    int x, y, i;
    char buf[16];

    x = (plotno % 2) ? graph->viewportxoff : graph->viewport.width / 2;
    y = graph->absolute.height - graph->fontheight - ((plotno + 2) / 2) * graph->fontheight;
    i = y + graph->fontheight / 2 + 1;
    SetColor(dv->v_color);
    if (graph->plottype == PLOT_POINT) {
        sprintf(buf, "%c : ", dv->v_linestyle);
        Text(buf, x + graph->viewport.width / 20 - 3 * graph->fontwidth, y);
    } else {
        SetLinestyle(dv->v_linestyle);
        DrawLine(x, i, x + graph->viewport.width / 20, i);
    }
    SetColor(1);
    Text(dv->v_name, x + graph->viewport.width / 20 + graph->fontwidth, y);
}
...
void gr_pmsg (char *text)
{
	...
	Text(text, currentgraph->viewport.width
		- (strlen(currentgraph->grid.xlabel) + 3) * currentgraph->fontwidth,
...
static void gr_resize_internal(GRAPH *graph)
{
    if (!graph->grid.xsized)
	    graph->viewport.width = graph->absolute.width - 1.4 * graph->viewportxoff;
    if (!graph->grid.ysized)
	    graph->viewport.height = graph->absolute.height - 2 * graph->viewportyoff;
    ...
    graph->datawindow.width  = graph->datawindow.xmax - graph->datawindow.xmin;
    graph->datawindow.height = graph->datawindow.ymax - graph->datawindow.ymin;
...
void gr_redraw(GRAPH *graph)
{
    ...
    for (link=graph->plotdata, plotno = 0; link; link = link->next, plotno++) {
      ...
      */
      ft_graf(link->vector, graph->onevalue ? (struct dvec *) NULL :
	(link->vector->v_scale ?  link->vector->v_scale : link->vector->v_plot->pl_scale), true);
    }
...
static void gr_restoretext(GRAPH *graph)
{
    struct _keyed *k;
...
static int iplot(struct plot *pl, int id)
{
    ...
    for (j = 0, v = pl->pl_dvecs; v; v = v->v_next)
        if (v->v_flags & VF_PLOT) j++;
    if (!j) return(0);
    if (ft_grdb) fprintf(cp_err, "Entering iplot, len = %d\n\r", len);
	...
	resumption = false;
        ...
        for (v = pl->pl_dvecs; v; v = v->v_next)
            if (v->v_flags & VF_PLOT) {
                ...
                if (lims[0] < ylims[0]) ylims[0] = lims[0];
                if (lims[1] > ylims[1]) ylims[1] = lims[1];
                if (!yl) yl = v->v_name;
            }
        if (ft_grdb)
            fprintf(cp_err, "iplot: after 5, xlims = %G, %G, ylims = %G, %G\n\r",
...
/*
        (void) gr_init((double *) NULL, (double *) NULL, x->v_name,
            pl->pl_title, (char *) NULL, j, xdelta, ydelta,
            GRID_LIN, plottype, x->v_name, yl, x->v_type, yt,
...
*/
	sprintf(commandline, "iplot %s", x->v_name);

        (void) gr_init(xlims, ylims, x->v_name,
            pl->pl_title, (char *) NULL, j, 0.0, 0.0,
            GRID_LIN, PLOT_LIN, x->v_name, yl, x->v_type, yt,
	...
	Input(&reqst, 0);
        ...
        dy = (isreal(x) ? x->v_realdata[len - 1] :
                realpart(&x->v_compdata[len - 1]));
        if (ft_grdb)
            fprintf(cp_err, "x = %G\n\r", dy);
        ...
        while (dy < currentgraph->data.xmin) {
            ...
            if (ft_grdb)
              fprintf(cp_err, "resize: xlo %G -> %G\n\r",
                  ...
                  currentgraph->data.xmin -
                    (currentgraph->data.xmax - currentgraph->data.xmin) * XFACTOR);
            currentgraph->data.xmin -=
		  (currentgraph->data.xmax - currentgraph->data.xmin) * XFACTOR;
        ...
        for (v = pl->pl_dvecs; v; v = v->v_next) {
            if (!(v->v_flags & VF_PLOT)) continue;
            dy = (isreal(v) ? v->v_realdata[len - 1] : realpart(&v->v_compdata[len - 1]));
            if (ft_grdb) fprintf(cp_err, "y = %G\n\r", dy);
		...
		if (ft_grdb)
			...
			currentgraph->data.ymin -
			    (currentgraph->data.ymax - currentgraph->data.ymin) * YFACTOR);
		currentgraph->data.ymin -=
			    (currentgraph->data.ymax - currentgraph->data.ymin) * YFACTOR;
            }
        ...
        if (changed) {
             ...
             * be done with curve interpolation, so it might look funny.
             */
            for (v = pl->pl_dvecs; v; v = v->v_next)
                if (v->v_flags & VF_PLOT) {
                    gr_point(v,
                    (isreal(x) ? x->v_realdata[len - 1] : realpart(&x->v_compdata[len - 1])),
                    (isreal(v) ? v->v_realdata[len - 1] : realpart(&v->v_compdata[len - 1])),
                    (isreal(x) ? x->v_realdata[len - 2] : realpart(&x->v_compdata[len - 2])),
                    (isreal(v) ? v->v_realdata[len - 2] : realpart(&v->v_compdata[len - 2])),
                    len - 1);
                }
        }
    }
...
void gr_iplot(struct plot *plot)
{
	...
	for (v = plot->pl_dvecs; v; v = v->v_next) {
		...
		if (len <= 1 || hit <= 0 || hit2 < 0) {
		    hit = 1;
		    hit2 = 1;
		    printf("\tExecution trace (remove with the \"delete\" command)");
		    printf("\n");

		    if (u) {
			printf("%12s:", u->v_name);
			if (isreal(u)) {
			    printf("%s", getitright(buf, u->v_realdata[len - 1]));
			} else {
			    /* MW. Complex data here, realdata is NULL
			    		(why someone use realdata here again) */
			    printf("%s", getitright(buf, u->v_compdata[len - 1].cx_real));
			    printf(", %s", getitright(buf, u->v_compdata[len - 1].cx_imag));
			}
			printf("\n");
		    }
		}
...
static double * readtics (char *string)
{
    ...
    tics = allocn(double, MAXTICS);
    ticsk = tics;
    words = string;

    for (i = k = 0; *words && k < MAXTICS; words = worde) {

	while (isspace(*words)) words++;

	worde = words;
	while (isalpha(*worde) || isdigit(*worde)) worde++;
