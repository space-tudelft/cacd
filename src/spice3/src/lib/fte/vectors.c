...
void vec_remove (char *name)
{
    ...
    for (ov = plot_cur->pl_dvecs; ov; ov = ov->v_next) {
        if (cieq(name, ov->v_name) && (ov->v_flags & VF_PERMANENT)) break;
    }
    if (!ov) return;
...
/* Get a vector by name. This deals with v(1), etc. almost properly.
 * Also, it checks for pre-defined vectors.
 */
 ...
 * In the other two cases, either the plot or the name can be "all", a wildcard.
...
static struct dvec * findvec (char *word, struct plot *pl)
{
    ...
    for (d = pl->pl_dvecs; d; d = d->v_next)
        if (cieq(word, d->v_name) && (d->v_flags & VF_PERMANENT)) break;
    if (!d) {
        sprintf(buf, "v(%s)", word);
        for (d = pl->pl_dvecs; d; d = d->v_next)
            if (cieq(buf, d->v_name) && (d->v_flags & VF_PERMANENT)) break;
    }
    if (d && d->v_link2) {
        d = vec_copy(d);
        vec_new(d);
...
struct dvec * vec_copy (struct dvec *v)
{
    ...
    if (isreal(v)) {
        nv->v_realdata = allocn(double, v->v_length);
        bcopy((char *) v->v_realdata, (char *) nv->v_realdata, sizeof (double) * v->v_length);
        nv->v_compdata = NULL;
    } else {
        nv->v_realdata = NULL;
        nv->v_compdata = allocn(complex, v->v_length);
        bcopy((char *) v->v_compdata, (char *) nv->v_compdata, sizeof (complex) * v->v_length);
    }
...
void vec_gc()
{
    ...
    for (pl = plot_list; pl; pl = pl->pl_next)
        for (d = pl->pl_dvecs; d; d = nd) {
            ...
            if (!(d->v_flags & VF_PERMANENT)) {
                if (ft_vecdb)
                    fprintf(cp_err, "vec_gc: throwing away %s.%s\n", pl->pl_typename, d->v_name);
                vec_free(d);
            }
        }
...
void vec_free (struct dvec *v)
{
    ...
    if (!v || !v->v_name) return;

    pl = v->v_plot;

    /* Now we have to take this dvec out of the plot list. */
    if (!pl) fprintf(cp_err, "vec_free: Internal Error: plot ptr is 0\n");
    ...
    if (pl->pl_dvecs == v)
        ...
        if (lv->v_next == NULL)
            fprintf(cp_err, "vec_free: Internal Error: %s not in plot\n", v->v_name);
        lv->v_next = v->v_next;
    }
    if (pl->pl_scale == v) {
        if (pl->pl_dvecs)
...
char * vec_basename (struct dvec *v)
{
    ...
    for (t = buf; *t; t++) if (isupper(*t)) *t = tolower(*t);
    for (t = buf; isspace(*t); t++) ;
    s = t;
    for (t = s; *t; t++) ;
    while ((t > s) && isspace(t[-1])) *--t = '\0';
    return (copy(s));
}
...
static int veccmp (struct dvec **d1, struct dvec **d2)
{
    ...
    if ((i = namecmp((*d1)->v_plot->pl_typename, (*d2)->v_plot->pl_typename))) return (i);
...
static int namecmp (char *s, char *t)
{
    ...
    for (;;) {
        while ((*s == *t) && !isdigit(*s) && *s) s++, t++;
        if (!*s) return (0);
        if ((*s != *t) && (!isdigit(*s) || !isdigit(*t))) return (*s - *t);

        /* The beginning of a number... Grab the two numbers
         * and then compare them...
         */
        for (i = 0; isdigit(*s); s++) i = i * 10 + (*s - '0');
        for (j = 0; isdigit(*t); t++) j = j * 10 + (*t - '0');
...
void vec_transpose (struct dvec *v)
{
    ...
    if (isreal(v)) {
	newreal = allocn(double, v->v_length);
	...
	for (k=0; k < nummatrices; k++) {
	    ...
	    for (j=0; j < dim0; j++) {
		for (i=0; i < dim1; i++) {
		  newreal[koffset + joffset + i] = oldreal[koffset + i*dim0 + j];
		}
	...
	newcomp = allocn(complex, v->v_length);
...
struct dvec * vec_mkfamily (struct dvec *v)
{
	...
	sprintf(buf, "%s%s", v->v_name, indexstring(count, v->v_numdims - 1));
        d->v_name = copy(buf);
        d->v_type = v->v_type;
        d->v_flags = v->v_flags;
