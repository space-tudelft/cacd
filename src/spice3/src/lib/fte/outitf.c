	...
	runDesc **runp)
{
    ...
    *runp = run = alloc(struct runDesc);
    ...
    if (numsaves) {
        savesused = allocn(bool, numsaves);
        saveall = false;
        ...
        savesused = NULL; /* prevent compiler warning */
    ...
    for (i = 0; i < numsaves; i++) {
		...
		saves[i].used = 1;
                ...
                depind = 0; /* prevent compiler warning */
    ...
    if (run->writeOut)
        ...
        plotInit(run);
	if (refName) run->runPlot->pl_ndims = 1;
    }
...
static int addDataDesc(runDesc *run, char *name, int type, int ind)
{
    ...
    if (!run->numData)
        ...
        run->data = (dataDesc *) trealloc((char *) run->data, sizeof(dataDesc) * (run->numData + 1));
...
int OUTattributes(GENERIC *plotPtr, char *varName, int param, IFvalue *value)
{
	...
	if (varName) {
		...
		if (eq(varName, d->v_name)) d->v_gridtype = type;
	} else {
...
static void fileInit_pass2(runDesc *run)
{
    ...
    for (i = 0; i < run->numData; i++) {
        ...
        sprintf(buf, "V(%s)", run->data[i].name);
          name = buf;
        } else {
...
static void plotAddRealValue(dataDesc *desc, double value)
{
    ...
    if (isreal(v)) {
      v->v_realdata = (double *) trealloc((char *) v->v_realdata, sizeof(double) * (v->v_length + 1));
      v->v_realdata[v->v_length] = value;
    } else {
      /* a real parading as a VF_COMPLEX */
      v->v_compdata = (complex *) trealloc((char *) v->v_compdata, sizeof(complex) * (v->v_length + 1));
...
static bool parseSpecial(char *name, char *dev, char *param, char *ind)
{
    ...
    *dev = *param = *ind = '\0';
    ...
    s = dev;
    while (*name && *name != '[') *s++ = *name++;
    *s = '\0';
    if (!*name) return (true);
    name++;

    s = param;
    while (*name && *name != ',' && *name != ']') *s++ = *name++;
    *s = '\0';
    if (*name == ']')
...
static bool name_eq (char *n1, char *n2)
{
    ...
    if ((s = strchr(n1, '('))) {
        strcpy(buf1, s);
        if (!(s = strchr(buf1, ')'))) return (false);
        *s = '\0';
        n1 = buf1;
    }
    if ((s = strchr(n2, '('))) {
        strcpy(buf2, s);
        if (!(s = strchr(buf2, ')'))) return (false);
        *s = '\0';
        n2 = buf2;
    }

    return (eq (n1, n2) ? true : false);
}
...
void OUTerror(int flags, char *format, IFuid *names)
{
    ...
    if ((flags == ERR_INFO) && cp_getvar("printinfo", VT_BOOL, (char *) &printinfo))
	return;

    for (m = msgs; m->flag; m++)
                if (flags & m->flag) fprintf(cp_err, "%s: ", m->string);
