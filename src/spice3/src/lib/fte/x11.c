...
static void handlebuttonev (Widget, caddr_t, caddr_t);
static void handlekeypressed (Widget, caddr_t, caddr_t);
static void hardcopy (Widget, caddr_t, caddr_t);
static void killwin (Widget, caddr_t, caddr_t);
static void redraw (Widget, caddr_t, caddr_t);
static void resize (Widget, caddr_t, caddr_t);
static void slopelocation (GRAPH *, int, int);
static void zoomin (GRAPH *);
...
static void initcolors(), initlinestyles();
...
static int errorhandler(Display *display, XErrorEvent *errorev)
{
...
#define RECOVERNEWVIEWPORT()    free((char *) graph); graph = (GRAPH *) NULL;
...
static void initcolors(GRAPH *graph)
{
	...
	for (i = 0; i < xmaxcolors; i++) {
	    ...
	    strcpy(colorstring, colornames[i]);
	    if (!XAllocNamedColor(display,
		    DefaultColormap(display, DefaultScreen(display)),
		    colorstring, &visualcolor, &exactcolor)) {
		sprintf(ErrorMessage, "can't get color %s\n", colorstring);
		externalerror(ErrorMessage);
...
static void handlekeypressed(Widget w, caddr_t clientdata, caddr_t calldata)
{
...
#ifdef notdef
void keyhandler(caddr_t clientdata, int *source, XtInputId id)
{
	KEYwaiting = TRUE;
}
#endif

static void handlebuttonev(Widget w, caddr_t clientdata, caddr_t calldata)
{
...
#ifdef notdef
handlemotionev(Widget w, caddr_t clientdata, caddr_t calldata)
{
...
static void slopelocation(GRAPH *graph, int x0, int y0) /* x0,y0 initial position of mouse */
{
	Window rootwindow, childwindow;
	int x1, y1, x, y, rootx, rooty;
	unsigned int state;
...
/* note: what are src_x, src_y, src_width, and src_height for? XXX */
	...
	if (fx0 > fx1) { ftemp = fx0; fx0 = fx1; fx1 = ftemp; }
	if (fy0 > fy1) { ftemp = fy0; fy0 = fy1; fy1 = ftemp; }

	strncpy(buf2, graph->plotname, sizeof(buf2));
	if ((t = strchr(buf2, ':'))) *t = 0;

	if (!eq(plot_cur->pl_typename, buf2)) {
	    sprintf(buf, "setplot %s; %s xlimit %le %le ylimit %le %le; setplot $curplot\n",
		buf2, graph->commandline, fx0, fx1, fy0, fy1);
	} else {
	    sprintf(buf, "%s xlimit %le %le ylimit %le %le\n", graph->commandline, fx0, fx1, fy0, fy1);
	}
	...
	if (cp_getvar("plothistory", VT_BOOL, (char *) &dummy)) {
	    wl = cp_lexer(buf);
	    if (wl && wl->wl_word) cp_addhistent(cp_event++, wl);
	    wl_free(wl);
	}
	(void) cp_evloop(buf);
}

static void hardcopy(Widget w, caddr_t client_data, caddr_t call_data)
{
	lasthardcopy = (GRAPH *) client_data;
	com_hardcopy(NULL);
}

static void killwin(Widget w, caddr_t client_data, caddr_t call_data)
{
...
static void redraw(Widget w, caddr_t client_data, caddr_t call_data)
{
	GRAPH *graph = (GRAPH *) client_data;
	...
	if (XCheckWindowEvent(display, DEVDEP(graph).window, StructureNotifyMask, &ev)) {
	...
	while (n < 30 && XCheckWindowEvent(display, DEVDEP(graph).window, ExposureMask, &ev)) {
	  pev = (XExposeEvent *) &ev;
	  rects[n].x = pev->x;
	  rects[n].y = pev->y;
...
static void resize(Widget w, caddr_t client_data, caddr_t call_data)
{
	GRAPH *graph = (GRAPH *) client_data;
	XEvent ev;
	...
	while (XCheckWindowEvent(display, DEVDEP(graph).window, /* ExposureMask | */ StructureNotifyMask, &ev)) ;
...
int X11_Input (REQUEST *request, RESPONSE *response)
{
	...
	switch (request->option) {
	  case char_option:
	    ...
	    nfds = ConnectionNumber(display) > fileno(request->fp) ?  ConnectionNumber(display) : fileno(request->fp);
	    ...
	    while (1) {
	      ...
	      readfds = 1 << fileno(request->fp) | 1 << ConnectionNumber(display);
...
out:
	if (response) response->option = request->option;
	return(0);
}
...
static void X_ScreentoData (GRAPH *graph, int x, int y, double *fx, double *fy)
{
		...
		|| graph->grid.gridtype == GRID_LOGLOG)
	{
		...
		*fx = (x - graph->viewportxoff) * graph->aspectratiox + graph->datawindow.xmin;
	}
...
static void linear_arc(int x0, int y0, int radius, double theta1, double theta2)
    ...
    /* theta1 = initial angle ( +x axis = 0 rad ) */
    /* theta2 = final angle ( +x axis = 0 rad ) */
    /*
     * Notes:
     ...
     */
{
    ...
    while (theta1 >= theta2) theta2 += 2 * M_PI;
