...
#define  IS_VALID(matrix)       ((matrix) != NULL && (matrix)->ID == SPARSE_ID &&   \
...
#define ALLOC(type,number) (type *)tmalloc(sizeof(type) * (number))
#define REALLOC(ptr,type,number) \
       ptr = (type *)trealloc((char *)ptr, sizeof(type) * (number))
#define FREE(ptr) { txfree((char *)(ptr)); (ptr) = NULL; }

/* Calloc that properly handles allocating a cleared vector. */
#define CALLOC(ptr,type,number) \
{   int i = number; ptr = ALLOC(type, i); \
    if (ptr) while (--i >= 0) ptr[i] = (type) 0; \
}
 ...
 *  options spCOMPLEX and spSEPARATED_COMPLEX_VECTORS are set.
 */
