...
static void ComplexTransposedMatrixMultiply(MatrixPtr, RealVector, RealVector, RealVector, RealVector);
#else
...
static int CountTwins(MatrixPtr Matrix, int Col, ElementPtr *ppTwin1, ElementPtr *ppTwin2)
{
    ...
    {   if (ABS(pTwin1->Real) == 1.0)
        {   Row = pTwin1->Row;
            ...
            while (pTwin2 AND pTwin2->Row != Col)
                pTwin2 = pTwin2->NextInCol;
            if (pTwin2 AND (ABS(pTwin2->Real) == 1.0))
...
static void SwapCols(MatrixPtr Matrix, ElementPtr pTwin1, ElementPtr pTwin2)
{
    ...
    for (e = Matrix->FirstInCol[Col1]; e; e = e->NextInCol)
	e->Col = Col2;
    for (e = Matrix->FirstInCol[Col2]; e; e = e->NextInCol)
	e->Col = Col1;
#endif
 ...
 */
#if spCOMPLEX AND spSEPARATED_COMPLEX_VECTORS
static void ComplexTransposedMatrixMultiply(MatrixPtr Matrix, RealVector RHS, RealVector Solution, RealVector iRHS, RealVector iSolution)
#else
static void ComplexTransposedMatrixMultiply(MatrixPtr Matrix, RealVector RHS, RealVector Solution)
#endif
{
    register ElementPtr pElement;
    register ComplexVector Vector;
...
/* Mark the fill-ins. */
    {   register ElementPtr pFillin, pLastFillin;
        ...
        {   pFillin = pListNode->pFillinList;
            ...
            while (pFillin <= pLastFillin) (pFillin++)->Row = 0;
...
void spErrorMessage (char *eMatrix, FILE *Stream, char *Originator)
{
    ...
    if (Originator[0]) fprintf(Stream, "%s: ", Originator);
    if (Error >= spFATAL)
