 ...
 */
        pElement = (RealNumber*)spcFindElementInCol(Matrix,
				     &Matrix->FirstInCol[Col], Row, Col, YES);
    }
    return pElement;
}
...
ElementPtr spcFindElementInCol(MatrixPtr Matrix,
    register ElementPtr *LastAddr, register int Row, int Col, BOOLEAN CreateIfMissing)
{
    register  ElementPtr  pElement;
...
ElementPtr spcCreateElement(MatrixPtr Matrix,
    int Row, register int Col, register ElementPtr *LastAddr, BOOLEAN Fillin)
{
...
static void EnlargeMatrix(MatrixPtr Matrix, register int NewSize)
{
    ...
    if (NewSize <= OldAllocatedSize) return;
