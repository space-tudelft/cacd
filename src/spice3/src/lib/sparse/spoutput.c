...
/* Create a packed external to internal row and column translation array. */
    ...
    if (!PrintOrdToIntRowMap)
    {   Matrix->Error = spNO_MEMORY;
	FREE(pImagElements);
        return;
    }
    CALLOC(PrintOrdToIntColMap, int, Top + 1);
    if (!PrintOrdToIntColMap)
    {   Matrix->Error = spNO_MEMORY;
	FREE(pImagElements);
...
/* ... in each column of the group. */
            ...
            {   if (PrintReordered)
                ...
                while(pElement AND pElement->Row != Row)
...
/* Open file matrix file in write mode. */
    if (!(pMatrixFile = fopen(File, "w"))) return 0;
...
#endif
{
    ...
    ASSERT(IS_SPARSE(Matrix) AND RHS)

/* Open File in append mode. */
    if (!(pMatrixFile = fopen(File, "a"))) return 0;
...
/* Open File in append mode. */
    if (!(pStatsFile = fopen(File, "a"))) return 0;
