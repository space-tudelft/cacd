 ...
 */
int spOrderAndFactor(char *eMatrix, RealNumber RHS[],
	RealNumber RelThreshold, RealNumber AbsThreshold, BOOLEAN DiagPivoting)
{
...
/* Set Count to -1 initially to remove count due to pivot element. */
        ...
        while (pElement AND pElement->Col < Step)
...
/* Set Count to -1 initially to remove count due to pivot element. */
        ...
        while (pElement AND pElement->Row < Step)
            pElement = pElement->NextInCol;
        while (pElement)
        {   Count++;
            pElement = pElement->NextInCol;
...
/* Singleton does not lie on diagonal, find it. */
            ...
            {   ChosenPivot = Matrix->FirstInCol[I];
                while ((ChosenPivot) AND (ChosenPivot->Row < Step))
		...
		{   /* Reduced column has no elements, matrix is singular. */
		    break;
                ...
                PivotMag = ELEMENT_MAG(ChosenPivot);
                if (PivotMag > Matrix->AbsThreshold AND
                     PivotMag > Matrix->RelThreshold * FindBiggestInColExclude(Matrix, ChosenPivot, Step)
                ) return ChosenPivot;
                else
                {   if (Matrix->MarkowitzRow[I] == 0)
                    {   ChosenPivot = Matrix->FirstInRow[I];
                        while(ChosenPivot AND ChosenPivot->Col < Step)
 ...
 * off diagonals are placed symmetricly. */
            if (pOtherInRow  AND  pOtherInCol)
...
/* This case handles Markowitz ties. */
            ...
            pElement = pElement->NextInCol;
        }  /* End of while(pElement) */
    } /* End of for(Step) */

    if (ChosenPivot) return ChosenPivot;
...
/* Exchange elements in rows while traveling from left to right. */
    ...
    }  /* end of while(Row1Ptr OR Row2Ptr) */
...
/* Exchange elements in rows while traveling from top to bottom. */
    ...
    }  /* end of while(Col1Ptr OR Col2Ptr) */
...
/* Element2 does not exist, move Element1 down to Row2. */
            if (ElementBelowRow1 AND ElementBelowRow1->Row < Row2)
            {
...
/* Search column for Row2. */
                ...
                }   while (pElement AND pElement->Row < Row2);
...
/* Element2 does not exist, move Element1 to right to Col2. */
            if (ElementRightOfCol1 AND ElementRightOfCol1->Col < Col2)
            {
/* Element1 must be removed from linked list and moved. */
...
/* Search Row for Col2. */
                ...
                }   while (pElement AND pElement->Col < Col2);
...
/* Update Markowitz numbers. */
    for (ColPtr = pPivot->NextInCol; ColPtr; ColPtr = ColPtr->NextInCol)
    {   Row = ColPtr->Row;
        --MarkoRow[Row];
...
/* Form Markowitz product while being cautious of overflows. */
    ...
    for (RowPtr = pPivot->NextInRow; RowPtr; RowPtr = RowPtr->NextInRow)
    {   Col = RowPtr->Col;
        --MarkoCol[Col];
