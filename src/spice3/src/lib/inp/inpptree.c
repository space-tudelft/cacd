    ...
    case PT_FUNCTION:
        ...
        case PTF_SQRT:  /* 1 / (2 * sqrt(u)) */
            arg1 = mkb(PT_DIVIDE, mkcon((double) 1), mkb(PT_TIMES,
                    mkcon((double) 2), mkf(PTF_SQRT, p->left)));
            break;
    ...
    while ((sp > 1) || (next->token != TOK_END)) {
        ...
        case G:
            ...
            if (st == sp) {
                ...
                if (lpn == NULL || stack[sp].type != TYP_STRING) goto err;
                if (!(pn = mkfnode(stack[sp].value.string, lpn))) return (NULL);
            } else { /* node op node */
 ...
 * is one, return a pointer to it). If it isn't of type VALUE, then return NULL.
 */
static INPparseNode * makepnode(PTelement *elem)
{
    if (elem->token != TOK_VALUE) return (NULL);
...
static INPparseNode * mkfnode(char *fname, INPparseNode *arg)
{
    ...
    if (eq(buf, "v")) {
        ...
        if (arg->type == PT_PLACEHOLDER) {
            ...
            sprintf(name, "%d", (int) arg->constant);
	    ...
	    for (i = 0; i < numvalues; i++)
		if (types[i] == IF_NODE && values[i].nValue == temp.nValue) break;
	    if (i == numvalues) {
		if (numvalues) {
...
/* printf("getting a device called '%s'\n", name); */
        ...
        for (i = 0; i < numvalues; i++)
            if (types[i] == IF_INSTANCE && values[i].uValue == temp.uValue) break;
        if (i == numvalues) {
            if (numvalues) {
                values = (IFvalue *) REALLOC((char *) values, (numvalues + 1) * sizeof(IFvalue));
                types = (int *) REALLOC((char *) types, (numvalues + 1) * sizeof(int));
...
static INPparseNode * mksnode(char *string)
{
    ...
    for (i = 0; i < ft_sim->numSpecSigs; i++)
        if (eq(ft_sim->specSigs[i], buf)) break;
    if (i < ft_sim->numSpecSigs) {
        for (j = 0; j < numvalues; j++)
            if (types[j] == IF_STRING && eq(buf, values[i].sValue)) break;
        if (j == numvalues) {
            if (numvalues) {
                values = (IFvalue *) REALLOC((char *) values, (numvalues + 1) * sizeof(IFvalue));
                types = (int *) REALLOC((char *) types, (numvalues + 1) * sizeof(int));
    ...
    for (i = 0; i < NUM_CONSTANTS; i++)
        if (eq(constants[i].name, buf)) break;
