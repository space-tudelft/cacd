...
int DCtran(CKTcircuit *ckt, int restart) /* forced restart flag */
{
    int i;
    double olddelta;
    double delta;
	...
	if (ltra_num >= 0 && ckt->CKThead[ltra_num])
	...
	/* end LTRA code addition */
        ...
        if (!ckt->CKTbreaks) return(E_NOMEM);
        ckt->CKTbreaks[0] = 0;
...
resume:
	...
	;
        ...
        if (ckt->CKTfinalTime/50<ckt->CKTmaxStep) {
            printf("limited by Tstop/50\n");
        } else {
            printf("limited by Tmax\n");
        }
    }
#endif
    ckt->CKTdelta = MIN(ckt->CKTdelta, ckt->CKTmaxStep);
	/* are we at a breakpoint, or indistinguishably close? */
    if ((ckt->CKTtime == ckt->CKTbreaks[0]) || (ckt->CKTbreaks[0] - ckt->CKTtime <= ckt->CKTdelmin)) {
	...
	mt = .1 * ckt->CKTsaveDelta;
	nt = .1 * (ckt->CKTbreaks[1] - ckt->CKTbreaks[0]);
	if (mt < ckt->CKTdelta || nt < ckt->CKTdelta) {
            if (mt < nt)  {
		ckt->CKTdelta = mt;
#ifdef STEPDEBUG
                printf("limited by pre-breakpoint delta\n");
#endif
            } else {
		ckt->CKTdelta = nt;
#ifdef STEPDEBUG
                printf("limited by next breakpoint\n");
#endif
            }
        }
        ...
        if (firsttime) {
            ckt->CKTdelta /= 10;
#ifdef STEPDEBUG
            printf("delta cut for initial timepoint\n");
#endif
        }
...
#ifdef STEPDEBUG
        printf("delta cut to hit breakpoint\n");
#endif
    ...
    while (1) {
        ...
        if (converged != 0) {
...
#ifdef STEPDEBUG
            printf("delta cut for non-convergence\n");
#endif
            if (firsttime) {
                ckt->CKTmode = (ckt->CKTmode & MODEUIC)|MODETRAN | MODEINITTRAN;
            }
            ...
            if (new > .9 * ckt->CKTdelta) {
                ...
                ckt->CKTdelta = new;
#ifdef STEPDEBUG
                printf("delta set to truncation error result:point accepted\n");
#endif
#ifdef HAS_SENSE2
                if (ckt->CKTsenInfo && (ckt->CKTsenInfo->SENmode & TRANSEN)) {
                    save1 = ckt->CKTmode;
                ...
                ckt->CKTdelta = new;
#ifdef STEPDEBUG
                printf("delta set to truncation error result:point rejected\n");
#endif
            }
        }
