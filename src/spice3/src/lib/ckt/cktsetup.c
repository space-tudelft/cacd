...
#define CKALLOC(var, size, type) \
    if (size && (!(var  = (type *)MALLOC((size)*sizeof(type))))) return(E_NOMEM)
...
int CKTsetup(register CKTcircuit *ckt)
{
    ...
    if (ckt->CKTisSetup) return E_NOCHANGE;
    ...
    for (i = 0; i < DEVmaxnum; i++) {
        if (DEVices[i]->DEVsetup && ckt->CKThead[i]) {
...
int CKTunsetup(CKTcircuit *ckt)
{
    ...
    for (i = 0; i < DEVmaxnum; i++) {
        if (DEVices[i]->DEVunsetup && ckt->CKThead[i]) {
