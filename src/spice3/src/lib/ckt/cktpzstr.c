...
extern int SMPcDProd (SMPmatrix *, SPcomplex *, int *);
static void clear_trials (int);
static void check_flat (PZtrial *, PZtrial *);
static void CKTpzUpdateSet (PZtrial **, PZtrial *);
static void CKTpzReset (PZtrial **);
#ifdef PZDEBUG
static void show_trial (PZtrial *, char);
#endif
...
int CKTpzFindZeros(CKTcircuit *ckt, PZtrial **rootinfo, int *rootcount)
{
    PZtrial *new_trial;
	...
	if ((*(SPfrontEnd->IFpauseTest))()) {
	    sprintf(ebuf, "Pole-Zero analysis interrupted; %d trials, %d roots\n", Seq_Num, NZeros);
...
#ifdef PZDEBUG
    DEBUG(1) fprintf(stderr, "Finished: NFlat %d, NZeros: %d, NTrials %d, Guess %g to %g, aber %d\n",
...
int PZeval(int strat, PZtrial *set[], PZtrial **new_trial_p)
{
	...
	if (Guess_Param > High_Guess) High_Guess = Guess_Param;
	if (Guess_Param < Low_Guess) Low_Guess = Guess_Param;
...
#ifdef PZDEBUG
		DEBUG(1) fprintf(stderr, "FIXED UP BAD Strat: %s (%d) was (%.15g,%.15g)\n",
		    snames[strat], CKTpzTrapped, new_trial->s.real, new_trial->s.imag);
#endif
	...
	if (NIpzK != 0.0 && NIpzK_mag > -10) {
	    while (NIpzK_mag > 0) { NIpzK *= 2.0; NIpzK_mag -= 1; }
	    while (NIpzK_mag < 0) { NIpzK /= 2.0; NIpzK_mag += 1; }
...
*/
int CKTpzRunTrial(CKTcircuit *ckt, PZtrial **new_trialp, PZtrial *set[])
{
    PZtrial	*match, *base, *new_trial;
    PZtrial	*p, *prev;
    SPcomplex	def_frac, diff_frac;
    double	reltol, abstol;
    int		def_mag, diff_mag, error, i;
    int		pretest, shifted, was_shifted;
    int		repeat;

    new_trial = *new_trialp;

    if (new_trial->s.imag < 0.0) new_trial->s.imag *= -1.0;
	...
	for (p = Trials; p; p = p->next) {
		...
		FABS(diff_frac.real) / (FABS(p->s.real) + abstol/reltol) < reltol) {
	...
	DEBUG(1) {
		...
		p->s.real, p->s.imag, new_trial->s.real, new_trial->s.imag, p->flags);
	}
#endif
	...
	error = E_SINGULAR; /* SdeG, prevent compiler warning */
...
int CKTpzVerify(PZtrial **set, PZtrial *new_trial)
{
    ...
    NZeros += 1;
    if (new_trial->s.imag != 0.0) NZeros += 1;
    NFlat = 0;
	...
	if (new_trial->s.imag != 0.0) C_MAG2(diff_frac);
	...
	if (t->s.imag != 0.0
	    ...
	    if (t->flags & ISAMINIMA) t->flags &= ~ISAMINIMA;
...
static PZtrial * pzseek(PZtrial *t, int dir)
{
    ...
    if (dir == 0 && !(t->flags & ISAROOT) && !(t->flags & ISAMINIMA)) return t;
...
static void CKTpzUpdateSet(PZtrial *set[3], PZtrial *new)
{
    int	this_move;
...
static void check_flat(PZtrial *a, PZtrial *b)
{
	...
	C_MAG2(diff_frac);
	if (diff_frac.real < 1.0e-20) NFlat += 1;
    }
	/* XXX else NFlat = ?????? */
}

/* XXX ZZZ */
int CKTpzStep(int strat, PZtrial *set[])
{
    switch (strat) {
    case INIT:
	if (!set[1]) {
...
static void CKTpzReset(PZtrial *set[3])
{
    CKTpzTrapped = 0;
    Consec_Moves = 0;
