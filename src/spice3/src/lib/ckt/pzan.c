...
int PZan(CKTcircuit *ckt, int reset)
{
    ...
    error = CKTload(ckt); /* Make sure that all small signal params are set */
    if (error) return(error);
...
int PZinit(CKTcircuit *ckt)
{
    ...
    if (i != -1 && ckt->CKThead[i])
	ERROR(E_XMISSIONL, "Transmission lines not supported")
...
int PZpost(CKTcircuit *ckt)
{
    ...
    namelist = (IFuid *) MALLOC((pzan->PZnPoles + pzan->PZnZeros)*sizeof(IFuid));
    out_list = (IFcomplex *)MALLOC((pzan->PZnPoles + pzan->PZnZeros)*sizeof(IFcomplex));
    ...
    j = 0;
    if (pzan->PZnPoles > 0) {
	for (root = pzan->PZpoleList; root; root = root->next) {
    ...
    if (pzan->PZnZeros > 0) {
	for (root = pzan->PZzeroList; root; root = root->next) {
	    for (i = 0; i < root->multiplicity; i++) {
		out_list[j].real = root->s.real;
		out_list[j].imag = root->s.imag;
		j += 1;
