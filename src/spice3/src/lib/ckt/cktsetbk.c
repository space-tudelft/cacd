...
int CKTsetBreak(register CKTcircuit *ckt, double time)
{
    ...
    if (ckt->CKTtime > time) {
        (*(SPfrontEnd->IFerror))(ERR_PANIC, "breakpoint in the past - HELP!", (IFuid *)NULL);
        return(E_INTERN);
    }
    ...
    ckt->CKTbreakSize++;
    ckt->CKTbreaks = (double *)REALLOC(ckt->CKTbreaks, ckt->CKTbreakSize * sizeof(double));
