...
void DISswap(double **a, double **b)
{
    double *c;
...
void DmemAlloc(double **a, int size)
{
    *a = (double *) MALLOC(sizeof(double) * size + 1);
}

void DstorAlloc(double ***header, int size)
{
...
int DISTOan(CKTcircuit *ckt, int restart)
{
    double freq;
...
time1 = (*(SPfrontEnd->IFseconds))();
	...
	freqTol = job->DfreqDelta * job->DstopF1 * ckt->CKTreltol;
	...
	break;
    case LINEAR:
	job->DfreqDelta = (job->DstopF1 - job->DstartF1) / (job->DnumSteps + 1);
...
time1 = (*(SPfrontEnd->IFseconds))();
	...
	{
	    DkerProc(D_TWOF1, job->r2H11stor[i], job->i2H11stor[i], size, job);
	...
	for (i = 0; i < displacement; i++)
	{
	    DkerProc(D_THRF1, job->r3H11stor[i], job->i3H11stor[i], size, job);
	...
	for (i = 0; i < displacement; i++)
	{
	    DkerProc(D_F1PF2, job->r2H12stor[i], job->i2H12stor[i], size, job);
	...
	for (i = 0; i < displacement; i++)
	{
	    DkerProc(D_F1MF2, job->r2H1m2stor[i], job->i2H1m2stor[i], size, job);
	...
	for (i = 0; i < displacement; i++)
	{
	    DkerProc(D_2F1MF2, job->r3H1m2stor[i], job->i3H1m2stor[i], size, job);
