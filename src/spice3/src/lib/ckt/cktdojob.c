...
int CKTdoJob(GENERIC *inCkt, int reset, GENERIC *inTask)
{
	...
	for (i = 0; i <  ANALmaxnum; i++)
	    if (eq("SENS2", analInfo[i]->public.name)) break;
	sens_num = i;
    }
#endif
	...
	/* normal reset */
	if (!error) error = CKTunsetup(ckt);
	if (!error) error = CKTsetup(ckt);
	if (!error) error = CKTtemp(ckt);
	...
	for (job = task->jobs; job; job = job->JOBnextJob) {
	    if (job->JOBtype == i) {
                ckt->CKTcurJob = job;
		error = OK;
		if (analInfo[i]->an_init) error = (*(analInfo[i]->an_init))(ckt, job);
		if (!error && analInfo[i]->do_ic) error = CKTic(ckt);
		if (!error) error = (*(analInfo[i]->an_func))(ckt, reset);
...
#ifdef HAS_SENSE2
    if (ckt->CKTsenInfo) SENdestroy(ckt->CKTsenInfo);
#endif
