...
int PZsetParm(CKTcircuit *ckt, GENERIC *anal, int which, IFvalue *value)
{
    ...
    case PZ_NODEK:
        ...
        break;
    case PZ_V:
        if (value->iValue) ((PZAN*)anal)->PZinput_type = PZ_IN_VOL;
        break;
    case PZ_I:
        if (value->iValue) ((PZAN*)anal)->PZinput_type = PZ_IN_CUR;
        break;
    case PZ_POL:
        if (value->iValue) ((PZAN*)anal)->PZwhich = PZ_DO_POLES;
        break;
    case PZ_ZER:
        if (value->iValue) ((PZAN*)anal)->PZwhich = PZ_DO_ZEROS;
        break;
    case PZ_PZ:
        if (value->iValue) ((PZAN*)anal)->PZwhich = PZ_DO_POLES | PZ_DO_ZEROS;
        break;
    default:
        return(E_BADPARM);
    }
