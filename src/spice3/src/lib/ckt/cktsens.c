...
static int count_steps(int, double, double, int, double *);
static double inc_freq(double, int, double);
extern int sens_getp(sgen *, CKTcircuit *, IFvalue *);
static int sens_load(sgen *, CKTcircuit *, int);
static int sens_setp(sgen *, CKTcircuit *, IFvalue *);
static int sens_temp(sgen *, CKTcircuit *);
extern void spConstMult(SMPmatrix *, double);
...
int sens_sens(CKTcircuit *ckt, int restart)
{
	...
	if (restart) {
		...
		if (!is_dc) freq = sen_info->start_freq;
		...
		for (sg = sgen_init(ckt, is_dc); sg; sgen_next(&sg)) {
			if (!sg->is_instparam) {
				sprintf(namebuf, "%s:%s",
					(char*)sg->instance->GENname,
					sg->ptable[sg->param].keyword);
			} else if ((sg->ptable[sg->param].dataType
				& IF_PRINCIPAL) && sg->is_principle == 1)
			{
				sprintf(namebuf, "%s", (char*)sg->instance->GENname);
			} else {
	...
	if (!sen_info->output_volt)
		...
		branch_eq = 0; /* prevent compiler warning */
	...
	for (i = 0; i < nfreqs; i++) {
			...
			sg; sgen_next(&sg))
		{
			...
			if ((error = sens_load(sg, ckt, is_dc))) {
			    if (error != E_BADPARM) return error; /* XXX */
			    continue;
			}
...
#ifdef ASDEBUG
			DEBUG(1) printf("New value: %g\n", nvalue.rValue);
#endif

			error = sens_setp(sg, ckt, &nvalue);
			if (error && error != E_BADPARM) return error;
...
#ifdef ASDEBUG
			DEBUG(1) {
			    if (sens_getp(sg, ckt, &value)) continue;
			...
			/* delta_Y E */
			spMultiply(delta_Y, delta_I_delta_Y, E, delta_iI_delta_Y, iE);
			...
			if (is_dc) {
				if (sen_info->output_volt)
					output_values[n] = delta_I[sen_info->output_pos->number]
						- delta_I[sen_info->output_neg->number];
				else {
				...
				if (sen_info->output_volt) {
					output_cvalues[n].real = delta_I[sen_info->output_pos->number]
						- delta_I[sen_info->output_neg->number];
					output_cvalues[n].imag = delta_iI[sen_info->output_pos->number]
						- delta_iI[sen_info->output_neg->number];
				} else {
					output_cvalues[n].real = delta_I[branch_eq];
					output_cvalues[n].imag = delta_iI[branch_eq];
				}
...
#if 0
double next_freq(int type, double freq, double stepsize)
{
	double	s;
