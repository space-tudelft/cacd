...
int CKTnoise (CKTcircuit *, int, int, Ndata *);

int NOISEan (CKTcircuit *ckt, int restart)
{
    register Ndata *data;
    double realVal;
    double imagVal;
    ...
    if (code != -1) {
        error = CKTfndDev((GENERIC*)ckt, &code, &inst, job->input, (GENERIC *)NULL, (IFuid)NULL);
    ...
    if (code != -1 && !inst) {
        ...
        if (error) {
	    /* XXX ??? */
            (*(SPfrontEnd->IFerror))(ERR_WARNING, "Noise input source %s not in circuit", &job->input);
	    return (E_NOTFOUND);
	}
	...
	error = (*(SPfrontEnd->OUTpBeginPlot))(ckt, (GENERIC *)(ckt->CKTcurJob),
	   ...
	   freqUid, IF_REAL, data->numPlots, data->namelist, IF_REAL, &data->NplotPtr);
	if (error) return(error);

        if (job->NstpType != LINEAR) {
	    (*(SPfrontEnd->OUTattributes))((GENERIC *)data->NplotPtr, NULL, OUT_SCALE_LOG, NULL);
	}
