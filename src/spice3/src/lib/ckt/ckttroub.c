...
char * CKTtrouble(GENERIC *cktp, char *optmsg)
{
    ...
    if (!ckt || !ckt->CKTcurJob) return NULL;
	...
	for (i = 0; i <= cv->TRCVnestLevel; i++) {
		...
		sprintf(msg_p, " %s = %g: ", (char*)cv->TRCVvName[i],
	...
	sprintf(msg_p, "trouble with node \"%s\"\n",
		(char*)CKTnodName(ckt, ckt->CKTtroubleNode));
    } else if (ckt->CKTtroubleElt) {
	/* "-" for dop */
	sprintf(msg_p, "trouble with %s-instance %s\n",
		(char*)ckt->CKTtroubleElt->GENmodPtr->GENmodName,
		(char*)ckt->CKTtroubleElt->GENname);
    } else {
