...
int DCtrCurv(CKTcircuit *ckt, int restart) /* forced restart flag */
{
    ...
    for (i = 0; i <= cv->TRCVnestLevel; i++) {
        ...
        (*(SPfrontEnd->IFerror))(ERR_FATAL, "DCtrCurv: source %s not in circuit", &cv->TRCVvName[i]);
        return(E_NODEV);
found:;
    }
    ...
    for (;;) {
        ...
        if (cv->TRCVvType[i] == vcode) { /* voltage source */
            if ((((VSRCinstance*)(cv->TRCVvElt[i]))->VSRCdcValue)*
                    SIGN(1., cv->TRCVvStep[i]) > SIGN(1., cv->TRCVvStep[i]) * cv->TRCVvStop[i])
                {
                    i++;
                    firstTime = 1;
                    ckt->CKTmode = (ckt->CKTmode & MODEUIC) | MODEDCTRANCURVE | MODEINITJCT;
        ...
        while (i > 0) {
            ...
            i--;
            if (cv->TRCVvType[i] == vcode) { /* voltage source */
                ((VSRCinstance *)(cv->TRCVvElt[i]))->VSRCdcValue = cv->TRCVvStart[i];
            } else if (cv->TRCVvType[i] == icode) { /* current source */
                ((ISRCinstance *)(cv->TRCVvElt[i]))->ISRCdcValue = cv->TRCVvStart[i];
            } /* else not possible */
        }
        ...
        if (cv->TRCVvType[0] == vcode) {
            ckt->CKTtime = ((VSRCinstance *)(cv->TRCVvElt[i]))->VSRCdcValue;
        } else if (cv->TRCVvType[0] == icode) {
            ckt->CKTtime = ((ISRCinstance *)(cv->TRCVvElt[i]))->ISRCdcValue;
        }
...
nextstep:;
        if (cv->TRCVvType[i] == vcode) { /* voltage source */
            ((VSRCinstance*)(cv->TRCVvElt[i]))->VSRCdcValue += cv->TRCVvStep[i];
        } else if (cv->TRCVvType[i] == icode) { /* current source */
            ((ISRCinstance*)(cv->TRCVvElt[i]))->ISRCdcValue += cv->TRCVvStep[i];
        } /* else not possible */
    ...
    for (i = 0; i <= cv->TRCVnestLevel; i++) {
        if (cv->TRCVvType[i] == vcode) {   /* voltage source */
            ((VSRCinstance*)(cv->TRCVvElt[i]))->VSRCdcValue = cv->TRCVvSave[i];
            ((VSRCinstance*)(cv->TRCVvElt[i]))->VSRCdcGiven = cv->TRCVgSave[i];
        } else /* if (cv->TRCVvType[i] == icode) current source */ {
            ((ISRCinstance*)(cv->TRCVvElt[i]))->ISRCdcValue = cv->TRCVvSave[i];
