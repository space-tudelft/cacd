...
/*  Arg vportargs[5]; */
    ...
    top->viewwidget = XtCreateManagedWidget("viewport",
            viewportWidgetClass, top->formwidget, vportargs, 5);
#endif

    /* rip out font changes */
    buf = tmalloc(80 * top->numlines + 100);
    i = 0;
    for (wl = top->text; wl; wl = wl->wl_next) {
	s = wl->wl_word;
	while (*s) {
	    if ((*s == '\033' && s[1]) || (*s == '_' && s[1] == '\b')) s += 2;
	    else buf[i++] = *s++;
	}
	buf[i++] = '\n';
    }
    ...
    top->textwidget = XtCreateManagedWidget("helptext",
           asciiTextWidgetClass, top->formwidget, htextargs, XtNumber(htextargs));
...
void newtopic (Widget w, caddr_t client_data, caddr_t call_data)
{
...
void delete (Widget w, caddr_t client_data, caddr_t call_data)
{
