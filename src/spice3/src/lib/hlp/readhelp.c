...
long findsubject (char *filename, char *subject)
{
    FILE *fp;
    char buf[BSIZE_SP];
...
static void sortlist (toplink **tlp)
{
    ...
    for (tl = *tlp; tl; tl = tl->next) num++;
    if (!num) return;
    vec = allocn(toplink *, num);
    for (tl = *tlp, i = 0; tl; tl = tl->next, i++) vec[i] = tl;
    qsort(vec, num, sizeof (toplink *), sortcmp);
    *tlp = vec[0];
    for (i = 0; i < num - 1; i++) vec[i]->next = vec[i+1];
    vec[i]->next = NULL;
    tfree(vec);
}

static int sortcmp(tlp1, tlp2)
    toplink **tlp1, **tlp2;
{
