...
int BSIM3temp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    {    Temp = ckt->CKTtemp;
	 ...
	 model->BSIM3vcrit = CONSTvt0 * log(CONSTvt0 / (CONSTroot2 * 1e-14));
         ...
         for (here = model->BSIM3instances; here; here = here->BSIM3nextInstance)
	 {
		  ...
		  {   Inv_L = 1e-6 / pParam->BSIM3leff;
		      Inv_W = 1e-6 / pParam->BSIM3weff;
		      Inv_LW = 1e-12 / (pParam->BSIM3leff * pParam->BSIM3weff);
		  }
		  else
		  ...
		  pParam->BSIM3vfbcv = model->BSIM3vfbcv
                  ...
                  pParam->BSIM3abulkCVfactor = 1.0 + pow((pParam->BSIM3clc
					     / pParam->BSIM3leffCV), pParam->BSIM3cle);
	          ...
	          pParam->BSIM3uc = pParam->BSIM3uc + pParam->BSIM3uc1 * T0;
                  if (pParam->BSIM3u0 > 1.0)
                      pParam->BSIM3u0 = pParam->BSIM3u0 / 1e4;
	      ...
	      /* SdeG. changed 'pParam' into 'here->pParam' */
	      here->BSIM3cgso = here->pParam->BSIM3cgso;
	      here->BSIM3cgdo = here->pParam->BSIM3cgdo;
              ...
              {   here->BSIM3vjdm = Nvtm * log(model->BSIM3ijth / DrainSatCurrent + 1.0);
                  here->BSIM3IsEvjdm = DrainSatCurrent * exp(here->BSIM3vjdm / Nvtm);
              }
         }
    }
    return(OK);
