...
int MOS2temp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    /* now model parameter preprocessing */
        ...
        if (model->MOS2substrateDopingGiven) {
            if (model->MOS2substrateDoping *1e6 /*(cm**3/m**3)*/ >1.45e16) {
                ...
                (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: Nsub < Ni", &model->MOS2modName);
                return(E_BADPARM);
            }
        }
        ...
        for (here = model->MOS2instances; here; here = here->MOS2nextInstance) {
            ...
            if (here->MOS2l - 2 * model->MOS2latDiff <= 0) {
                (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: effective channel length less than zero", &here->MOS2name);
            }
            ...
            if (here->MOS2tSatCurDens == 0 || here->MOS2drainArea == 0 || here->MOS2sourceArea == 0) {
