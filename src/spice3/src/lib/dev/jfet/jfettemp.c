...
int JFETtemp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    for ( ; model; model = model->JFETnextModel) {
        ...
        egfet1 = 1.16 - (7.02e-4 * model->JFETtnom * model->JFETtnom) / (model->JFETtnom + 1108);
        ...
        if (model->JFETdepletionCapCoeff > .95) {
            (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: Depletion cap. coefficient too large, limited to .95", &model->JFETmodName);
	...
	/* end Sydney University mod */
        ...
        for (here = model->JFETinstances; here; here = here->JFETnextInstance) {
            ...
            egfet = 1.16-(7.02e-4*here->JFETtemp*here->JFETtemp)/ (here->JFETtemp+1108);
            ...
            here->JFETcorDepCap = model->JFETdepletionCapCoeff * here->JFETtGatePot;
