...
int ISRCtemp(GENmodel *inModel, CKTcircuit *ckt)
{
        ...
        for (here = model->ISRCinstances; here; here = here->ISRCnextInstance) {
            ...
            if (!here->ISRCdcGiven) {
                ...
                if (here->ISRCfuncTGiven) {
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, "Source %s has no DC value, transient time 0 value used", &here->ISRCname);
                } else {
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, "Source %s has no value, DC 0 assumed", &here->ISRCname);
                }
            }
