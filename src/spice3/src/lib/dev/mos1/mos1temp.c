...
int MOS1temp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    /* now model parameter preprocessing */
                ...
                if (model->MOS1substrateDoping*1e6 /*(cm**3/m**3)*/ > 1.45e16) {
                    ...
                    if (!model->MOS1vt0Given) {
                        ...
                        model->MOS1vt0 = vfb + model->MOS1type *
                                (model->MOS1gamma * sqrt(model->MOS1phi) + model->MOS1phi);
                    }
                } else {
                    model->MOS1substrateDoping = 0;
                    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: Nsub < Ni", &model->MOS1modName);
                    return(E_BADPARM);
                }
            }
        }
        ...
        for (here = model->MOS1instances; here; here = here->MOS1nextInstance) {
            ...
            egfet = 1.16-(7.02e-4*here->MOS1temp*here->MOS1temp)/ (here->MOS1temp+1108);
            ...
            if (here->MOS1l - 2 * model->MOS1latDiff <= 0) {
                (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: effective channel length less than zero", &model->MOS1modName);
            }
            ...
            here->MOS1tVbi = model->MOS1vt0 - model->MOS1type *
            ...
            if (here->MOS1tSatCurDens == 0 || here->MOS1drainArea == 0 || here->MOS1sourceArea == 0) {
                here->MOS1sourceVcrit = here->MOS1drainVcrit = vt*log(vt/(CONSTroot2*here->MOS1tSatCur));
            } else {
                here->MOS1drainVcrit = vt * log(vt / (CONSTroot2 *
                        here->MOS1tSatCurDens * here->MOS1drainArea));
                here->MOS1sourceVcrit = vt * log(vt / (CONSTroot2 *
