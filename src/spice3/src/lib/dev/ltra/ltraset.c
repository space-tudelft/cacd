...
int LTRAsetup(SMPmatrix *matrix, GENmodel *inModel, CKTcircuit *ckt, int *state)
{
	...
	if (!model->LTRAnlGiven) model->LTRAnl = .25;
	if (!model->LTRAfGiven) model->LTRAf = 1e9;
	if (!model->LTRAreltolGiven) model->LTRAreltol = 1;
	if (!model->LTRAabstolGiven) model->LTRAabstol = 1;

	if (!model->LTRAresistGiven) {
	    (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: lossy line series resistance not given, assumed zero", &model->LTRAmodName);
	    model->LTRAresist = 0.0;
	    /*return(E_BADPARM);*/
	}

	if (model->LTRAstLineReltol == 0.0) model->LTRAstLineReltol = ckt->CKTreltol;
	if (model->LTRAstLineAbstol == 0.0) model->LTRAstLineAbstol = ckt->CKTabstol;
	...
	if (model->LTRAhowToInterp != LTRA_MOD_LININTERP &&
	    ...
	    /*
	    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: have to specify one of lininterp, quadinterp or mixedinterp", &model->LTRAmodName);
	    return(E_BADPARM);
	    */
	    if (ckt->CKTtryToCompact) {
		model->LTRAhowToInterp = LTRA_MOD_LININTERP;
		(*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: using linear interpolation because trytocompact option specified", &model->LTRAmodName);
	    } else {
		model->LTRAhowToInterp = LTRA_MOD_QUADINTERP;
	    }
	}
	...
	if (!model->LTRAconductGiven) {
			    /*
	    (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: lossy line parallel conductance not given, assumed zero", &model->LTRAmodName);
			    */
	    model->LTRAconduct = 0.0;
	    /*return(E_BADPARM);*/
	}
	if (!model->LTRAinductGiven) {
	    (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: lossy line series inductance not given, assumed zero", &model->LTRAmodName);
	    model->LTRAinduct = 0.0;
	    /*return(E_BADPARM);*/
	}
	if (!model->LTRAcapacGiven) {
	    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: lossy line parallel capacitance not given, assumed zero", &model->LTRAmodName);
	    model->LTRAcapac = 0.0;
	    /*return(E_BADPARM);*/
	}
	if (!model->LTRAlengthGiven) {
	    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: lossy line length must be given", &model->LTRAmodName);
	    return(E_BADPARM);
	}
...
#ifdef LTRADEBUG
	    (*(SPfrontEnd->IFerror))(ERR_INFO, "%s: lossless line", &model->LTRAmodName);
#endif
	}
...
#ifdef LTRADEBUG
	    (*(SPfrontEnd->IFerror))(ERR_INFO, "%s: RLC line", &model->LTRAmodName);
#endif
	}
...
#ifdef LTRADEBUG
	    (*(SPfrontEnd->IFerror))(ERR_INFO, "%s: RC line", &model->LTRAmodName);
#endif
	}
	    ...
	    model->LTRAcapac == 0 && model->LTRAinduct != 0)
	{
	    ...
	    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: RL line not supported yet", &model->LTRAmodName);
	    return(E_BADPARM);
#ifdef LTRADEBUG
#endif
	}
...
#ifdef LTRADEBUG
	    (*(SPfrontEnd->IFerror))(ERR_INFO, "%s: RG line", &model->LTRAmodName);
#endif
	}

	if (model->LTRAconduct != 0 && (model->LTRAcapac != 0 || model->LTRAinduct != 0))
	{
	    model->LTRAspecialCase = LTRA_MOD_LTRA;
	    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: Nonzero G (except RG) line not supported yet", &model->LTRAmodName);
	    return(E_BADPARM);
#ifdef LTRADEBUG
#endif
	}

	if ((model->LTRAresist == 0.0? 0:1) + (model->LTRAconduct == 0.0? 0:1)
	    + (model->LTRAinduct == 0.0?0:1) + (model->LTRAcapac == 0.0? 0:1) <= 1) {
	    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: At least two of R,L,G,C must be specified and nonzero", &model->LTRAmodName);
	    return(E_BADPARM);
	}
...
int LTRAunsetup(GENmodel *inModel, CKTcircuit *ckt)
{
#ifndef HAS_BATCHSIM
    LTRAmodel *model;
    LTRAinstance *here;

    for (model = (LTRAmodel *)inModel; model; model = model->LTRAnextModel)
    {
        for (here = model->LTRAinstances; here; here = here->LTRAnextInstance)
	{
	    if (here->LTRAbrEq1) {
