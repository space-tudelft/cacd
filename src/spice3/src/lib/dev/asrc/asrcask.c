...
int ASRCask(CKTcircuit *ckt, GENinstance *instPtr, int which, IFvalue *value, IFvalue *select)
{
    ASRCinstance *here = (ASRCinstance*)instPtr;

    switch (which) {
    case ASRC_CURRENT:
	value->tValue = here->ASRCtype == ASRC_CURRENT ? here->ASRCtree : NULL;
	return(OK);
    case ASRC_VOLTAGE:
	value->tValue = here->ASRCtype == ASRC_VOLTAGE ? here->ASRCtree : NULL;
	return(OK);
    case ASRC_POS_NODE:
	...
	value->rValue = ckt->CKTrhsOld[here->ASRCposNode] - ckt->CKTrhsOld[here->ASRCnegNode];
	return(OK);
    }
    return(E_BADPARM);
}
