 ...
 */
int ASRCsetup(register SMPmatrix *matrix, GENmodel *inModel, register CKTcircuit *ckt, int *states)
{
...
if (!(here->ptr = SMPmakeElt(matrix, here->first, ((CKTnode*)(second))->number))) return(E_NOMEM)
            ...
            for (i = 0; i < here->ASRCtree->numVars; i++) {
                ...
                case IF_INSTANCE:
                    ...
                    if (here->ASRCtype == ASRC_VOLTAGE) {
                        /* CCVS */
                        if (v_first) {
                            here->ASRCposptr = (double **)REALLOC(here->ASRCposptr, (sizeof(double *) * (j+5)));
                            ...
                            here->ASRCposptr = (double **)REALLOC(here->ASRCposptr, (sizeof(double *) * (j+1)));
                            TSTALLOC(ASRCposptr[j++], ASRCbranch, ASRCcont_br);
                        }
                    } else if (here->ASRCtype == ASRC_CURRENT) {
                        /* CCCS */
			here->ASRCposptr = (double **)REALLOC(here->ASRCposptr, (sizeof(double *) * (j+2)));
...
int ASRCunsetup(GENmodel *inModel, CKTcircuit *ckt)
{
#ifndef HAS_BATCHSIM
    ASRCmodel *model;
    ASRCinstance *here;

    for (model = (ASRCmodel *)inModel; model; model = model->ASRCnextModel)
    {
        for (here = model->ASRCinstances; here; here = here->ASRCnextInstance)
	{
	    if (here->ASRCbranch) {
