...
int URCsetup(SMPmatrix *matrix, GENmodel *inModel, CKTcircuit *ckt, int *state)
{
	...
	if (!model->URCkGiven) model->URCk = 1.5;
	if (!model->URCfmaxGiven) model->URCfmax = 1e9;
	if (!model->URCrPerLGiven) model->URCrPerL = 1000;
	if (!model->URCcPerLGiven) model->URCcPerL = 1e-12;
...
/* may need to put in limits:  k>=1.1, freq <=1e9, rperl >=.1 */
        ...
        for (here = model->URCinstances; here; here = here->URCnextInstance)
	{
            ...
            if (model->URCisPerLGiven) {
                ...
                error = CKTmodCrt((GENERIC *)ckt, dtype, (GENERIC**)&modfast, dioUid);
                if (error) return(error);
                ptemp.rValue = c1;
                ...
                if (error) return(error);
                modfast = (GENmodel *)NULL;
                error = CKTmodCrt((GENERIC *)ckt, ctype, (GENERIC**)&modfast, capUid);
                if (error) return(error);
            }
            ...
            for (i = 1; i <= here->URClumps; i++) {
                ...
                if (i == here->URClumps) {
                    ...
                    namelo = MALLOC(10);
                    sprintf(namelo, "lo%d", i);
                    error = CKTmkVolt(ckt, (CKTnode**)&nodelo, here->URCname, namelo);
                    if (error) return(error);
                    lowr = nodelo;
                }
                r = prop*r1;
                ...
                if (model->URCisPerLGiven) {
                    /* use diode */
                    nameelt = MALLOC(10);
                    sprintf(nameelt, "dlo%d", i);
                    error = (*(SPfrontEnd->IFnewUid))(ckt, &eltUid,
                            here->URCname, nameelt, UID_INSTANCE, (GENERIC **)NULL);
                    if (error) return(error);
                    ...
                    if (error) return(error);
                    ptemp.rValue = c;
                    error = CKTpName("capacitance", &ptemp, ckt, ctype, nameelt, &fast);
                    if (error) return(error);
                }
...
int URCunsetup(GENmodel *inModel, CKTcircuit *ckt)
{
#ifndef HAS_BATCHSIM
	IFuid	varUid;
	int	error;
