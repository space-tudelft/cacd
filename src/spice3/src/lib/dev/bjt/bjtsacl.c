    ...
    CKTcircuit *ckt;
{
    ...
    double vbcOp;
    double A0;
    double DELA = 0; /* init, prevent compiler warning */
    double Apert;
    double DELAinv;
    double vte;
    double gcpr;
...
load:
            ...
            printf("ccs %.7e + j%.7e, cbc = %.7e + j%.7e", ccs, iccs, cbc, icbc);
            printf("cbe %.7e + j%.7e, cce = %.7e + j%.7e\n", cbe, icbe, cce, icce);
            printf("cc = %.7e + j%.7e, ce = %.7e + j%.7e, ", cc, icc, ce, ice);
            printf("ccprm = %.7e + j%.7e, ceprm = %.7e + j%.7e", ccprm, iccprm, ceprm, iceprm);
            printf("cb = %.7e + j%.7e, cbprm = %.7e + j%.7e, ", cb, icb, cbprm, icbprm)
            printf("cs = %.7e + j%.7e\n", cs, ics);
#endif /* SENSDEBUG */
