...
/* Default value Processing for B1 MOSFET Models */
        ...
        Cox = 3.453e-13/(model->B1oxideThickness * 1e-4); /* in F/cm**2 */
        ...
        for (here = model->B1instances; here; here = here->B1nextInstance) {
            ...
            if ((EffChanLength = here->B1l - model->B1deltaL * 1e-6) <= 0) {
                ...
                (*(SPfrontEnd->IFerror))(ERR_FATAL, "B1: mosfet %s, model %s: Effective channel length <=0", namarray);
                return(E_BADPARM);
            }
            if ((EffChanWidth = here->B1w - model->B1deltaW * 1e-6) <= 0) {
                ...
                namarray[0] = model->B1modName;
                namarray[1] = here->B1name;
                (*(SPfrontEnd->IFerror))(ERR_FATAL, "B1: mosfet %s, model %s: Effective channel width <=0", namarray);
                return(E_BADPARM);
            }
            ...
            if ((here->B1drainConductance = model->B1sheetResistance * here->B1drainSquares) != 0.0) {
                here->B1drainConductance = 1. / here->B1drainConductance;
            }

            /* process source series resistance */
            if ((here->B1sourceConductance = model->B1sheetResistance * here->B1sourceSquares) != 0.0) {
                here->B1sourceConductance = 1. / here->B1sourceConductance;
            }

            Leff = EffChanLength * 1e6; /* convert into micron */
            Weff = EffChanWidth  * 1e6; /* convert into micron */
