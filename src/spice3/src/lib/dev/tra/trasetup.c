...
if (!(here->ptr = SMPmakeElt(matrix, here->first, here->second))) return(E_NOMEM);
            ...
            if (!here->TRAimpedGiven) {
                (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: transmission line z0 must be given", &here->TRAname);
                return(E_BADPARM);
            }
        }
    }
...
int TRAunsetup(GENmodel *inModel, CKTcircuit *ckt)
{
#ifndef HAS_BATCHSIM
    TRAmodel *model;
    TRAinstance *here;

    for (model = (TRAmodel *)inModel; model; model = model->TRAnextModel)
    {
        for (here = model->TRAinstances; here; here = here->TRAnextInstance)
	{
	    if (here->TRAbrEq1) {
