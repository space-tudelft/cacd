...
int DIOtemp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    for ( ; model; model = model->DIOnextModel) {
        ...
        if (model->DIOgradingCoeff > .9) {
            (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: grading coefficient too large, limited to 0.9", &model->DIOmodName);
            model->DIOgradingCoeff = .9;
        }
        /* limit activation energy to min of .1 */
        if (model->DIOactivationEnergy < .1) {
            (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: activation energy too small, limited to 0.1", &model->DIOmodName);
            model->DIOactivationEnergy = .1;
        }
        /* limit depletion cap coeff to max of .95 */
        if (model->DIOdepletionCapCoeff > .95) {
            (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: coefficient Fc too large, limited to 0.95", &model->DIOmodName);
        ...
        for (here = model->DIOinstances; here; here = here->DIOnextInstance) {
            ...
            if (model->DIObreakdownVoltageGiven){
                ...
                if (cbv < here->DIOtSatCur*model->DIObreakdownVoltage/vt){
                    ...
                    emsg = MALLOC(100);
                    if (!emsg) return(E_NOMEM);
                    sprintf(emsg, "%%s: breakdown current increased to %g to resolve incompatability", cbv);
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, emsg, &here->DIOname);
                    FREE(emsg);
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, "with specified saturation current", (IFuid*)NULL);
                    xbv = model->DIObreakdownVoltage;
                } else {
                    tol = ckt->CKTreltol*cbv;
                    xbv = model->DIObreakdownVoltage-vt*log(1+cbv/ here->DIOtSatCur);
                    iter = 0;
                    for (iter = 0; iter < 25; iter++) {
                        xbv = model->DIObreakdownVoltage-vt*log(cbv/ here->DIOtSatCur+1-xbv/vt);
                        xcbv = here->DIOtSatCur*(exp((model->DIObreakdownVoltage -xbv)/vt)-1+xbv/vt);
                        if (FABS(xcbv-cbv) <= tol) goto matched;
                    }
                    emsg = MALLOC(100);
                    if (!emsg) return(E_NOMEM);
                    sprintf(emsg, "%%s: unable to match forward and reverse diode regions: bv = %g, ibv = %g", xbv, xcbv);
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, emsg, &here->DIOname);
                    FREE(emsg);
                }
                matched:
