...
int MOS6temp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    /* now model parameter preprocessing */
        ...
        if (!model->MOS6oxideThicknessGiven || model->MOS6oxideThickness == 0) {
            ...
            if (model->MOS6substrateDopingGiven) {
                if (model->MOS6substrateDoping*1e6 /*(cm**3/m**3)*/ >1.45e16) {
                    ...
                    (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: Nsub < Ni", &model->MOS6modName);
                    return(E_BADPARM);
                }
            }
        }
        ...
        for (here = model->MOS6instances; here; here = here->MOS6nextInstance) {
            ...
            if (here->MOS6l - 2 * model->MOS6latDiff <= 0) {
                (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: effective channel length less than zero", &model->MOS6modName);
            }
            ...
            here->MOS6tVbi = model->MOS6vt0 - model->MOS6type *
                        (model->MOS6gamma* sqrt(model->MOS6phi))
                    +.5*(egfet1-egfet)
            ...
            if ((here->MOS6tSatCurDens == 0) ||
                ...
                here->MOS6sourceVcrit = here->MOS6drainVcrit = vt*log(vt/(CONSTroot2*here->MOS6tSatCur));
            } else {
                here->MOS6drainVcrit = vt * log(vt / (CONSTroot2 *
                        here->MOS6tSatCurDens * here->MOS6drainArea));
                here->MOS6sourceVcrit = vt * log(vt / (CONSTroot2 *
            ...
            if (model->MOS6drainResistanceGiven) {
                ...
                if (!here->MOS6drainSquaresGiven || here->MOS6drainSquares == 0) {
                    here->MOS6drainSquares = 1;
                }
                if (model->MOS6sheetResistance != 0) {
                    here->MOS6drainConductance = 1 / (model->MOS6sheetResistance * here->MOS6drainSquares);
                } else {
                    here->MOS6drainConductance = 0;
                }
            } else {
            ...
            if (model->MOS6sourceResistanceGiven) {
                ...
                if (!here->MOS6sourceSquaresGiven || here->MOS6sourceSquares == 0) {
                    here->MOS6sourceSquares = 1;
                }
                if (model->MOS6sheetResistance != 0) {
                    here->MOS6sourceConductance = 1 / (model->MOS6sheetResistance * here->MOS6sourceSquares);
                } else {
                    here->MOS6sourceConductance = 0;
                }
            } else {
