        ...
        for (here = model->VSRCinstances; here; here = here->VSRCnextInstance) {
            ...
            if (!here->VSRCdcGiven) {
                ...
                if (here->VSRCfuncTGiven) {
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: no DC value, transient time 0 value used", &here->VSRCname);
                } else {
                    (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: has no value, DC 0 assumed", &here->VSRCname);
                }
            }
