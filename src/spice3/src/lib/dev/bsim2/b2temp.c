...
/* Default value Processing for B2 MOSFET Models */
        ...
        model->B2Cox  = 3.453e-13/(model->B2tox * 1e-4); /* in F/cm**2 */
	...
	pLastKnot = NULL;
	    ...
	    while (pSizeDependParamKnot && Size_Not_Found)
	    ...
	    if (Size_Not_Found)
	    {   here->pParam = (struct bsim2SizeDependParam *)malloc(sizeof(struct bsim2SizeDependParam));
                if (pLastKnot == NULL)
		...
		EffectiveLength = here->B2l - model->B2deltaL * 1e-6;
		EffectiveWidth  = here->B2w - model->B2deltaW * 1e-6;

                if (EffectiveLength <= 0)
		{  IFuid namarray[2];
                   namarray[0] = model->B2modName;
                   namarray[1] = here->B2name;
                   (*(SPfrontEnd->IFerror))(ERR_FATAL, "B2: mosfet %s, model %s: Effective channel length <=0", namarray);
                   return(E_BADPARM);
                }
		...
		{  IFuid namarray[2];
                   namarray[0] = model->B2modName;
                   namarray[1] = here->B2name;
                   (*(SPfrontEnd->IFerror))(ERR_FATAL, "B2: mosfet %s, model %s: Effective channel width <=0", namarray);
                   return(E_BADPARM);
                }
		...
		here->pParam->Phis3 = here->pParam->SqrtPhi
            ...
            if ((here->B2drainConductance = model->B2sheetResistance * here->B2drainSquares) != 0.0) {
                here->B2drainConductance = 1. / here->B2drainConductance;
            }

            /* process source series resistance */
            if ((here->B2sourceConductance = model->B2sheetResistance * here->B2sourceSquares) != 0.0) {
