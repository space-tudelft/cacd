...
int MOS3temp(GENmodel *inModel, CKTcircuit *ckt)
{
    ...
    for ( ; model; model = model->MOS3nextModel) {
        ...
        egfet1 = 1.16-(7.02e-4*model->MOS3tnom*model->MOS3tnom)/ (model->MOS3tnom+1108);
        ...
        if (model->MOS3substrateDopingGiven) {
            if (model->MOS3substrateDoping*1e6 /*(cm**3/m**3)*/ >1.45e16) {
                ...
                (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: Nsub < Ni", &model->MOS3modName);
                return(E_BADPARM);
            }
        }
    /* now model parameter preprocessing */
        ...
        for (here = model->MOS3instances; here; here = here->MOS3nextInstance) {
            ...
            if (here->MOS3l - 2 * model->MOS3latDiff <= 0) {
                (*(SPfrontEnd->IFerror))(ERR_FATAL, "%s: effective channel length less than zero", &here->MOS3name);
                return(E_BADPARM);
            }
