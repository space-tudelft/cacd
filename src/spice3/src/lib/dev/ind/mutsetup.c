...
int MUTsetup(register SMPmatrix *matrix, GENmodel *inModel, CKTcircuit *ckt, int *states)
{
    ...
    for ( ; model; model = model->MUTnextModel) {
        ...
        for (here = model->MUTinstances; here; here = here->MUTnextInstance) {
            ...
            if (ktype <= 0) {
                (*(SPfrontEnd->IFerror))(ERR_PANIC, "mutual inductor, but inductors not available!", (IFuid *)NULL);
                return(E_INTERN);
            }
            ...
            if (error) {
                ...
                (*(SPfrontEnd->IFerror))(ERR_WARNING, "%s: coupling to non-existant inductor %s.", namarray);
            }
