        ...
        if (1) { /* } */
            ...
            if (ckt->CKTniState & NISHOULDREORDER) {
                ...
                if (error) {
                    ...
                    message = MALLOC(1000); /* should be enough */
                    sprintf(message, "singular matrix:  check nodes %s and %s\n",
                            (char*)CKTnodName(ckt, i), (char*)CKTnodName(ckt, j));
...
/*XXXX*/
        ...
        if (ckt->CKTmode & MODEINITFLOAT) {
            ...
            if (ckt->CKTnoncon == 0) ckt->CKTmode = (ckt->CKTmode & ~INITF)|MODEINITFLOAT;
            ipass = 1;
