...
int NIpzSym(PZtrial *set[3], PZtrial *new)
{
#ifndef notdef
    ...
    zaddeq(&a, &a_mag, set[1]->f_def.real, set[1]->mag_def, -set[0]->f_def.real, set[0]->mag_def);
    a /= dx0;
    zaddeq(&b, &b_mag, set[2]->f_def.real, set[2]->mag_def, -set[1]->f_def.real, set[1]->mag_def);
    b /= dx1;
...
int NIpzComplex(PZtrial *set[3], PZtrial *new)
{
    return NIpzSym2(set, new);
#ifdef notdef
    NIpzMuller(set, new);
...
int NIpzSym2(PZtrial *set[3], PZtrial *new)
{
    double a, b, c, x0, x1;
    double dx0, dx1, dx2, diff, disc;
    int  a_mag, b_mag, c_mag;
    int  tmag, disc_mag, new_mag;

    /* NIpzK = 0.0; NIpzK_mag = 0; */
    ...
    zaddeq(&c, &c_mag, b, b_mag, -a, a_mag);
    if (c != 0.0 && (dx2 = x1 - x0) != 0.0) {
	tmag = 0; R_NORM(dx2, tmag);
	c /= dx2; /* = f'' */
	c_mag -= tmag;
	R_NORM(c, c_mag);
    }

    if (c == 0.0 || ((a == 0.0 || c_mag < a_mag - 40) && (b == 0.0 || c_mag < b_mag - 40))) {
	...
	if (a != 0.0) {
	    ...
	    a_mag -= set[1]->mag_def;
	    while (a_mag > 0) { new->s.real /= 2.0; a_mag -= 1; }
	    while (a_mag < 0) { new->s.real *= 2.0; a_mag += 1; }
...
#ifdef PZDEBUG
		DEBUG(1) fprintf(stderr, "Ignore NIpzK: %g*2^%d for previous value of %g*2^%d\n",
		    disc, disc_mag, NIpzK, NIpzK_mag);
#endif
	    }
	    disc = 0.0;
	    disc_mag = 0;
	...
	while (b_mag > 0) { b *= 2.0; --b_mag; }
	while (b_mag < 0) { b /= 2.0; ++b_mag; }

	while (c_mag > 0) { c *= 2.0; --c_mag; }
	while (c_mag < 0) { c /= 2.0; ++c_mag; }
