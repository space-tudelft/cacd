...
struct model_info *findFunc (char *name, int imported, DM_PROJECT *father_proj, int flag)
{
	...
	PALLOC (low, 1, int); low[0] = 0;
	PALLOC (up,  1, int); up[0] = upper;
    ...
    fun -> dkey = dmCheckOut (fun -> proj, fun -> orig_name, ACTUAL, DONTCARE, CIRCUIT, READONLY);

    dsp = dmOpenStream (fun -> dkey, "fterm", "r");
    termRun (fun, dsp);
