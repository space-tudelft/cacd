...
# outline
   .cv create rectangle $xl $yt $xr $yb\
                        -fill wheat -tag ex210_$N_CELL
# terminals
   .cv create rectangle [expr $xl+$GW] [expr $y0-0.5*$GW]\
                        $xl [expr $y0-1.5*$GW]\
                        -fill wheat3 -tag "ex210_$N_CELL ex210_$N_CELL.A"
   .cv create rectangle [expr $xl+$GW] [expr $y0+0.5*$GW]\
                        $xl [expr $y0+1.5*$GW]\
                        -fill wheat3 -tag "ex210_$N_CELL ex210_${N_CELL}.B"
   .cv create rectangle [expr $xr-$GW] [expr $y0+$GW/2.]\
                        $xr [expr $y0-$GW/2.]\
                        -fill wheat3 -tag "ex210_$N_CELL ex210_$N_CELL.Y"
...
# symbol
   ...
   set CELL_ARR($N_CELL) ex210_$N_CELL
}
