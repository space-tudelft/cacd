...
# outline
   .cv create rectangle $xl $yt $xr $yb\
                        -fill wheat -tag na210_$N_CELL
# terminals
   .cv create rectangle [expr $xl+$GW] [expr $y0-0.5*$GW]\
                        $xl [expr $y0-1.5*$GW]\
                        -fill wheat3 -tag "na210_$N_CELL na210_$N_CELL.A"
   .cv create rectangle [expr $xl+$GW] [expr $y0+0.5*$GW]\
                        $xl [expr $y0+1.5*$GW]\
                        -fill wheat3 -tag "na210_$N_CELL na210_${N_CELL}.B"
   .cv create rectangle [expr $xr-$GW] [expr $y0+$GW/2.]\
                        $xr [expr $y0-$GW/2.]\
                        -fill wheat3 -tag "na210_$N_CELL na210_$N_CELL.Y"
...
# symbol
   ...
   .cv create line      [expr $x0+$GW] [expr $y0-$GW]\
                        [expr $x0+2*$GW] $y0\
                        -fill wheat3  -tag na210_$N_CELL
   .cv create text      $x0 $y0 -text "&" -font $CELL_FNT\
                        -fill wheat3  -tag na210_$N_CELL

   set CELL_ARR($N_CELL) na210_$N_CELL
}
