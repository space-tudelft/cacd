...
main ()
{
    char line[DM_MAXPATHLEN];	/* Room for the input line */
...
void hdestroy () /* Reset the module to its initial state */
{
    free ((void *) table);
#ifdef OPEN
    count = 0;
...
ENTRY *hsearch (ENTRY item, ACTION action) /* Find or insert the item into the table */
...
#ifdef DEBUG
    if (action == ENTER) printf("hash2 = %o\n", c);
#endif
...
ENTRY *hsearch (ENTRY item, ACTION action) /* Chained search with sorted lists */
/* item   = Item to be inserted or found */
/* action = FIND or ENTER */
{
...
static ENTRY *build (NODE **last, NODE *next, ENTRY item)
...
#ifdef DIV
static unsigned int hashd (POINTER key) /* Division hashing scheme */
...
static unsigned int hashm (POINTER key) /* Multiplication hashing scheme */
/* key - Key to be hashed */
{
...
static unsigned int hash2m (POINTER key) /* Secondary hashing routine */
...
static unsigned int crunch (POINTER key) /* Convert multicharacter key to unsigned int */
{
    unsigned int sum = 0;	/* Results */
    int s;			/* Length of the key */

    for (s = 0; *key; s++) sum += *key++; /* Simply add up the bytes */

    return(sum + s);
}

#ifdef DRIVER
static void hdump () /* Dumps loc, data, probe count, key */
{
	...
	if (table[i].key == NULL)
	    ...
	    sum += prcnt;
	    printf("%o.\t%d,\t%d,\t%s\n", i, *table[i].data, prcnt, table[i].key);
	    prcnt = oldpr;
	}
