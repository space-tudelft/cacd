...
static int getAttrXY (struct net_el *n)
{
    char *val;
    n -> x = INF_COORD;
    if ((val = getAttrValue (cnet.net_attribute, "x"))) {
	sscanf (val, "%d", &n -> x);
	if (n -> x == INF_COORD) return 1;
	if (!(val = getAttrValue (cnet.net_attribute, "y"))) return 1;
	sscanf (val, "%d", &n -> y);
    }
    return 0;
}
...
static void readNet (struct model_info *m, struct net_ref **anets)
{
 ...
 // dsp = dmOpenStream (m -> dkey, "net", "r");
	...
	dsp = dmOpenStream (m -> dkey, "netsub", "r");
	subfp = dsp -> dmfp;
    }
    else {
	dsp = dmOpenStream (m -> dkey, "net", "r");
    ...
    *anets = NULL;
	...
	nref -> n = n1;
	if (tanner) {
	    if (getAttrXY (n1)) fatalErr ("net_attribute x,y not found in circuit:", m -> name);
	}
	cpCnet2Netel (0, n1);

	if (subfp) { /* how many subnets? */
	    long pos = ftell (subfp);
	    nodeCounter = cnet.net_neqv;
	    n1 -> nx = nodeCounter++;
	    Neqv = 0;
	    while (dmGetDesignData (dsp, CIR_NET_ATOM) > 0 && cnet.net_neqv == 0) ++Neqv;
	    fseek (subfp, pos, SEEK_SET); /* rewind */
	}
	else {
	    Neqv = cnet.net_neqv;
	...
	if ((n1 -> net_neqv = Neqv) > 0) {
		...
		if (tanner) {
		    if (getAttrXY (n)) fatalErr ("net_attribute x,y not found in circuit:", m -> name);
		    if (n -> x == INF_COORD) {
			n -> x = n1 -> x;
			n -> y = n1 -> y;
		    }
