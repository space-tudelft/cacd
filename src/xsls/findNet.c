...
void findNetInit (struct model_info *ntw, struct net_ref *nets)
{
	...
	if (++j >= netCounter - 1) {
	    if (!nSUB && strcmp (eqv -> net_name, nameSUB) == 0) nSUB = eqv;
	    else
	    if (!nGND && strcmp (eqv -> net_name, nameGND) == 0) nGND = eqv;
	}
	...
	if ((i = eqv -> net_neqv) >= 0) {
	    ...
	    *hteladdr = eqv;
	    ...
	    if (subfp && !useNodeNrs && !eqv -> inst_name) { /* terminal or label */
...
struct net_el *findNet (struct net_el *eqv)
{
	...
	if (eqv -> net_name[0] == 'p') {
		...
		if (!useNodeNrs) nt = findTerm (n -> nx);
		return (nt ? nt : n);
	    }
	    else if (*s == 'G') { ++s;
		if (*s == '_')    { return (nGND); }
	...
	if ((n -> inst_name == inst_name
	    ...
	    if (useNodeNrs) n -> inst_name = inst_name;
	    return (n);
	}
next:
	n = n -> next;
