...
static void groupLibs ()
{
    struct cir *p1, *p2, *pp, *np;

    np = pp = NULL;
    for (p1 = beg_cirl; p1; p1 = (pp = p1) -> next) {
	if (p1 -> imported) {
	    p2 = p1 -> next;
	    if (p1 != beg_cirl) {
		p1 -> next = beg_cirl;
...
nextlib:
		...
		if (p2 -> proj == p1 -> proj) {
		    ...
		    pp = p2;
		    if (p2 -> imported && !np) np = p2;
		}
		p2 = pp -> next;
	    }
	    if (np) {
		for (pp = p1; pp -> next != np; pp = pp -> next) ;
		p2 = np -> next;
		if (pp != p1) {
		    np -> next = p1 -> next;
		    p1 -> next = np;
...
struct cir *cirTree (char *name)
{
	...
	end_cirl -> next = top;
	if (alsoImport || dialectCds) groupLibs ();
    }
