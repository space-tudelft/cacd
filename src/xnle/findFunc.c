...
extern struct model_info *allFuncs;
extern struct model_info *allpreFuncs;
...
struct model_info *findFunc (char *name, int imported, DM_PROJECT *father_proj, int flag)
{
	...
	for (fun = preFuncs; fun; fun = fun -> next) {
		...
		if (fun -> terms -> t -> term_upper[0] != upper) continue;
		return (fun);
	    }
	}
	...
	if (allpreFuncs) {
	    ...
	    for (fun = allpreFuncs; fun; fun = fun -> next) {
		if (fun -> name == prefuncs[i]
		 && fun -> terms -> t -> term_upper[0] == upper) {
		    if (mp) mp -> next = fun -> next;
		    else allpreFuncs = fun -> next;
	...
	if (ninputs > 1) {
	    char buf[12];
	    sprintf (buf, "%s%d", name, ninputs);
	    fun -> orig_name = strsave (buf);
	}
	if (flast) flast -> next = fun;
	else preFuncs = fun;
	flast = fun;
    ...
    if (allFuncs) {
	struct model_info *mp = NULL;
	for (fun = allFuncs; fun; fun = fun -> next) {
	    if (strcmp (fun -> name, name) == 0) {
		if (mp) mp -> next = fun -> next;
		else allFuncs = fun -> next;
		fun -> next = Funcs;
		return (Funcs = fun); /* found */
	    }
	    mp = fun;
	}
    }
