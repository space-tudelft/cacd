...
static void findChildren (char *name, int isroot, DM_PROJECT *proj, int fatherImported)
{
	...
	if (imported == IMPORTED) {
		...
		fatalErr ("Cannot handle aliases when also retrieving imported cells:", icp -> cellname);
...
static struct cir *locateTopCell (char *name)
{
	...
	if (*--s == 'e' && *--s == '_') {
	    *s = 0;
	    readPorts (dmproject, name);
	    *s = '_';
	}
	else if (isupper ((int)*name)) {
	    *name = tolower (*name);
	    readPorts (dmproject, name);
	    *name = toupper (*name);
	}
...
static void reportExt (char *name, int imported, DM_PROJECT *father_proj)
{
    ...
    if (allExts) {
	struct cir *mp = NULL;
	for (extl = allExts; extl; extl = extl -> next) {
	    if (strcmp (extl -> name, name) == 0) {
		if (mp) mp -> next = extl -> next;
		else allExts = extl -> next;
		if (end_extl) end_extl -> next = extl;
		else beg_extl = extl;
		extl -> next = NULL;
		end_extl = extl;
		return;
