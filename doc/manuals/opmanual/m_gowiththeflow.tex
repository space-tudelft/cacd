...
\hyphenation{GoWithTheFlow}
\title{Handleiding GoWithTheFlow}
\maketitle

\section{Inleiding}
Het programma \tool{GoWithTheFlow}
...
Tevens geeft \tool{GoWithTheFlow}
...
de mogelijkheden van \tool{GoWithTheFlow} beschrijven.
Voor een beschrijving van de werking van de afzonderlijke
programma's die door \tool{GoWithTheFlow} worden opgestart, zie
de handleidingen van deze programma's.

Het programma \tool{GoWithTheFlow} wordt gestart door op het desbetreffende icon
...
Nadat het programma \tool{GoWithTheFlow}
is opgestart verschijnt een window zoals weergegeven in figuur \ref{main-window}.
\begin{figure}[htb]
  \centerline{\callpsfig{GoWithTheFlow_hotelend.eps}{width=13cm}}
  \caption{Het main-window van het programma \tool{GoWithTheFlow}}
  \label{main-window}
\end{figure}
...
\section{De project subdirectories}
Wanneer een project directory wordt aangemaakt zal \tool{GoWithTheFlow}
...
\item {\bf components}: waarin de netwerkcomponenten worden geplaatst, die door
                  ...
                  door het programma \tool{GoWithTheFlow} om op een gemak\-kelijke
...
\section{De menubalk van het main-window}
In de menubalk van het main-window van \tool{GoWithTheFlow}
...
\item {\bf New entity}: Via dit commando kan een nieuwe entity worden
               ...
               \begin{figure}[htb]
               \centerline{\callpsfig{newhotelent.eps}{width=.40\textwidth}}
...
\item {\bf New schematic}: Start de schematic-editor \tool{schentry},
...
\item {\bf New package}: Door dit item te kiezen wordt er een basis-structuur voor
                  ...
                  \caption{New package-window van het programma \tool{GoWithTheFlow}}
...
\item {\bf Import project cell}: Via dit commando kunnen de gegevens die via \tool{GoWithTheFlow} in
                  ...
                  \caption{Het Import cell-window van het programma \tool{GoWithTheFlow}}
                  \label{copy-window}
                  \end{figure}
                  \begin{figure}[htb]
                  \centerline{\callpsfig{copy_wdw1.ps}{width=8cm}}
                  \caption{Ge"expandeerd Import cell-window van het programma \tool{GoWithTheFlow}}
                  \label{copy-window1}
                  \end{figure}
...
\item {\bf Quit}: Quit \tool{GoWithTheFlow}.
\end{itemize}
...
\caption{Het Generate Command file-window van het programma \tool{GoWithTheFlow}}
\label{mk-cmd-window}
\end{figure}
...
\caption{Het Add\_bond\_bar-window van het programma \tool{GoWithTheFlow}}
...
Door het klikken op \button{OK} wordt de schakeling met de bond\_bar aan het circuit
...
\begin{figure}[htb]
\centerline{\callpsfig{compare.eps}{width=14cm}}
\caption{Het Compare-window van het programma \tool{GoWithTheFlow}}
...
\item {\bf Print Layout}: Via dit commando kan een PostScript-file worden gemaakt van de
                    ...
                    \caption{Print\_Layout-window van het programma \tool{GoWithTheFlow}}
...
\item {\bf Reset project dir}: Door dit item te kiezen, wordt de directory waarin wordt
                    ...
                    Tijdens het werken met \tool{GoWithTheFlow} 
...
De letter-grootte in alle windows van het programma \tool{GoWithTheFlow} zal dan
aan de gekozen grootte worden aangepast.

\section{Het Package-menu}
Dit menu wordt getoond wanneer in het rechter midden-gedeelte van \tool{GoWithTheFlow}
...
\caption{Het Synthese-window van het programma \tool{GoWithTheFlow}}
\label{synth-window}
\end{figure}
...
\caption{Het Simulate-window van het programma \tool{GoWithTheFlow}}
...
\caption{Het Simulate\_dac-window van het programma \tool{GoWithTheFlow}}
\label{dac-window}
\end{figure}
...
\item {\bf Extract vhdl}: Via dit commando kan de \smc{vhdl}-code uit de layout worden ge"extraheerd.
  ...
  \caption{Het Extract-\smc{vhdl}-window van het programma \tool{GoWithTheFlow}}
...
\caption{Het make\_ldm-window van het programma \tool{GoWithTheFlow}}
\label{ldm-window}
\end{figure}
...
\label{edit-section}
Voor het genereren en veranderen van \smc{vhdl}-files wordt in \tool{GoWithTheFlow} een
...
\begin{figure}[htb]
\centerline{\callpsfig{hotelG.vhd.eps}{width=12cm}}
\caption{Het edit-window van het programma \tool{GoWithTheFlow}}
\label{edit-window}
\end{figure}
